...
| Harness Engineering | AI 활용 방법론. 워크플로우 설계, 오케스트레이션 | 19 |
| Tokenomics | AI 토큰 경제학. RTK, .claudeignore, 캐시 최적화 | 3 |
...
**11/11 카테고리 커버 완료.** 총 57 엔트리, 96개 주제 풀에서 AI가 매일 추천.
...
2026-03 ─────────────────────────────────────────────────
  │
  ├─ 🌱 v0.1 위키 기초
  │     Next.js 15 + MDX + force-directed 그래프
  │     10 카테고리 정의, 첫 엔트리 작성
  │
  ├─ 🤖 AI 과외 선생님 파이프라인
  │     GitHub Actions cron → Gemini가 매일 MDX 생성
  │     지식 그래프 분석 기반 주제 추천 알고리즘
  │
  ├─ 📊 학습 대시보드
  │     히트맵 + 스트릭 + 퀴즈 + Spaced Repetition
  │

2026-04 초 ──────────────────────────────────────────────
  │
  ├─ 📓 Harness Journal 시리즈 시작 (J000-J010)
  │     AI 하네스 엔지니어링 실전 기록
  │     CI 게이트, 자동 머지, squash merge 함정 패턴 박제
  │
  ├─ 🔗 허브-워커 모델 확립 (J011)
  │     ai-study(허브) ↔ moneyflow/tarosaju(워커)
  │     /projects-sync + /wt-branch 슬래시 커맨드
  │     사고 재발률 0/18+ 사이클
  │
  ├─ 🛡️ Runtime Validation 시리즈 (J012-J018)
  │     moneyflow 10/10 에이전트 Layer 1-4 완료
  │     type guard vs Zod 3차 재평가 → type guard 최종
  │     retryWithValidation + instruction augmentation
  │
  ├─ 🏷️ Tokenomics 카테고리 신설 (11번째)
  │     RTK: 47.4M tokens 절감 (99.5%)
  │     .claudeignore: context ~50% 축소
  │     cache read 98%+
  │
  ├─ ⚖️ LLM-as-a-Judge 3 프로젝트 적용
  │     moneyflow: 투자 분석 4차원 평가
  │     tarosaju: 운세/타로 4차원 평가
  │     ai-study: evaluation 패턴 박제
  │
  └─ 📈 현재: 57 entries · 18 journals · 11 categories
        Vibe Coding 쇼케이스 3 프로젝트
        허브-워커 양방향 복리 구조 운영 중
```
...
├── .claudeignore              # 컨텍스트 최적화 (~50% 축소)
├── .claude/
│   ├── commands/
│   │   ├── compound.md        # /compound — 복리형 지식 축적
│   │   ├── autoceo.md         # /autoceo — 풀 자동 스프린트 루프
│   │   ├── ingest.md          # /ingest — 외부 콘텐츠 가공
│   │   ├── projects-sync.md   # /projects-sync — 워커 프로젝트 진단
│   │   └── wt-branch.md       # /wt-branch — worktree 안전 분기
