...
| 3 | [aidy-ios로 다시 보는 iOS 흐름 맵 — TCA + SwiftUI + Tuist 역방향 학습](/wiki/ios-ai/ios-flow-map-via-aidy-ios) | ios-ai | aidy-ios (TCA + SwiftUI + Tuist) | 2026-04-16 |
| 4 | [aidy-architect로 보는 멀티 세션 오케스트레이션 — 한 명이 3-client 풀스택을 병렬로 짓는 법](/wiki/harness-engineering/architect-flow-map-via-aidy-architect) | harness-engineering | aidy-architect (Markdown specs + tmux) | 2026-04-16 |
| 6 | [API 계약이 코드보다 먼저 — api-contract.md 한 장으로 3-client 동기화](/wiki/context-engineering/api-contract-as-3-client-source-of-truth) | context-engineering | aidy-architect (specs/api-contract.md) | 2026-04-16 |
| 7 | [aidy 3-client 테스트 전략 — '컴파일 성공 ≠ 테스트 통과' 오신 끊기](/wiki/evaluation/test-strategy-3-client-via-aidy) | evaluation | aidy-architect (gates/test-policy*.md) | 2026-04-16 |

### 📝 예정

| # | 가제 | 카테고리 | 소스 레포 | 상태 |
|---|------|---------|-----------|------|
| 5 | aidy 풀스택 배포 파이프라인 — 로컬 → Neon/Vercel → 프로덕션 | infrastructure | (통합) | **deferred** — 실제 프로덕션 파이프라인 미구축. Neon/Vercel 도입 시점에 재개 |
