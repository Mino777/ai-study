...
```
        v1 (그라인딩 시대)              v2 (AI 시대·리서치 교정)
   ┌──────────────────────┐     ┌──────────────────────┐
   │ 🧮 DSA        60% ███ │     │ 🏛️ 아키텍처/SD   35% │
   │ 🍎 iOS 심화   25% █   │  →  │ 🍎 iOS 심화       25% │  ↑ SwiftUI 승격
   │ 🏛️ 아키텍처   15%     │     │ 🤖 AI 활용        20% │  🆕(iOS는 fluency로)
   │                       │     │ 🧮 DSA 쪽집게     12% │  ↓ 축소
   └──────────────────────┘     │ 💼 딥다이브/행동  8%  │
                                 └──────────────────────┘
```
...
2. 🍎 **iOS 심화 (25%)** — 동시성 **≒** SwiftUI(둘 다 시니어 감별점, SwiftUI는 이제 레드플래그 리스크). 강점 아닌 갭이라 배분 ↑
3. 🤖 **AI 활용 (20%)** — ⚠️ iOS는 전용 AI 라운드 아직 표준 아님 → **"AI 숙련도 서사화(fluency)"** 중심. 하네스 경험이 무기. `AI-TEST.md`
4. 🧮 **DSA 쪽집게 (12%)** — 라이브코딩 게이트용 **최소 시그니처 문제만** SRS 반복 (그라인딩 X)
5. 💼 **프로젝트 딥다이브 방어 + 행동면접 (8%·상시)** — AI 활용 스토리 포함. `interview-stories/`
...
## 🍎 iOS 심화 (동시성 ≒ SwiftUI — 둘 다 감별점)

> 🚩 **리서치 교정(2026):** SwiftUI가 "감 되살리기 서브"가 아니라 **필수 baseline**. *"2026에 SwiftUI 모르면 2019에 Auto Layout 모르는 격."* `@Observable vs ObservableObject` 설명 못 하면 **명시적 레드플래그.** 네 갭(스탯 30)이라 배분 격상.

1. **동시성(baseline·보너스 아님):** Rx 브릿지 → async/await → actor/Sendable/data race → structured concurrency → @MainActor/Swift6 (+ Task locals)
2. **SwiftUI(레드플래그 방어·필수):**
   - ⭐ **`@Observable`(iOS17+) vs `ObservableObject`** — 이거 못 하면 감점. 리렌더 추적 차이·성능
   - property wrapper 정확 구분: `@State`/`@Binding`/`@StateObject`/`@ObservedObject`
   - NavigationStack, UIKit interop(강점 활용), "새 화면은 SwiftUI가 기본" + 마이그레이션 경로 서사
...
**일일(평일 ~75분·5블록):**
🏛️ 아키텍처/SD(~20) + 🍎 iOS 심화(~18, 동시성/SwiftUI) + 🤖 AI 활용(~15) + 🧮 DSA 쪽집게(~10, 트리거 인출) + 🔁 SRS due 카드(~5·상시)
> 시간 부족 시 트리아지: 🏛️ 사수 > 🍎 iOS > 🤖 AI > 🧮. DSA는 "패턴 인출"만이라도 매일.
...
우선순위: **🏛️ 아키텍처/SD > 🍎 iOS(동시성·SwiftUI) > 🤖 AI 활용 > 🧮 DSA 트리거인출.**
