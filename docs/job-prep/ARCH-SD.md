...
- [x] B1. **프레임 소개 + per-user 규모추정** — ✅ 완료(2026-07-28, 냅킨 계산 실습 포함) → `lessons/sd-b1-napkin-math.md`. 기준값5 + 4스텝 + **이미지 디코딩 배율(해상도×4B, 파일크기 무관)** + 캐시 2계층·cost상한·다운샘플링
- [x] B2. **이미지 피드 (최빈출)** — ✅ 10-스텝 첫 완주(2026-07-28) → `lessons/sd-b2-image-feed.md`. 스코핑~마무리 + Deep Dive 3(이미지 파이프라인·오프라인 좋아요·60fps) + 트레이드오프 + **SSOT 압박 방어**
- [ ] B3. 무한 스크롤 타임라인 (cursor 페이지네이션·백엔드 노드32 연결)
- [x] B4. **오프라인 우선 노트앱** — ✅ 완주(2026-07-29, 강의형) → `lessons/sd-b4-offline-notes-crdt.md`. 편집단위 3택(문서/블록/문자)→충돌알고리즘 매핑 + **CRDT 3성질**(교환·결합·멱등) + tombstone 좀비부활 + **HLC**(기기시계 불신) + 순서병합이 텍스트병합보다 어려움 + **⭐충돌은 협업이 아니라 `오프라인 × replica≥2`가 만든다**
...
## 🚗 M 트랙 — 모빌리티 도메인 시나리오 (⭐ 내 실무 = 최강 무기)

> **신설 2026-07-29.** B 사다리가 SNS/이커머스 편향이라 **내 도메인이 0개**였다. Reverse SD("네가 만든 걸 설명하라")가 빈번해진 2026 트렌드에서 여기가 최대 우위.
> 상세 전체: **`lessons/sd-mobility-scenarios.md`** (per-user 예산 모빌리티판 · 특화 체크리스트 13축 · 도메인 압박질문 5)

- [x] M1. 🗺️ **실시간 차량 위치 지도** — ✅ **초심층 완료**(2026-08-03) → `lessons/sd-m1-map-deep.md`. L0 좌표계·Mercator 왜곡 → L1 타일/zoom 예산표 → L2 공간인덱스 4종 → L3 뷰포트쿼리 5단방어 → L4 클러스터링+안정성 → L5 렌더 16.6ms → L6 스트리밍 승격사다리 → L7 GPS 품질 → L8 거리 → L9 권한 → L10 배터리 → L11 SDK/오프라인 → L12 관측. **+ 실무 코드 대조 8건** · 압박방어 6 · 레드플래그 10
- [x] M2. 💳 **예약 → 결제** — ✅ **초심층 완료**(2026-08-03) → `lessons/sd-m2-payment-deep.md`. **exactly-once 불가능 → at-least-once+멱등** · 멱등키 계약 6조 + DB 유니크 최종방어 · 상태머신(**ambiguous를 상태로**) · 이중결제 4중방어 · **실패 3분류(모호=조회)** · **hold→capture 2단**(카셰어링 핵심) · **웹뷰 결제 아키텍처**(내 실무) · 미납 게이트 · **+ 실무 코드 대조 4건** · 압박방어 6
- [x] M3+M5. 🚙 **주행 데이터 + 요금** — ✅ **초심층 완료**(2026-08-03) → `lessons/sd-m3-m5-telemetry-fare-deep.md`. 🔴 **오답 교정: 카셰어링 앱은 주행 GPS를 기록하지 않는다**(백그라운드 위치 0건 · 주행거리는 서버 필드) → **차량이 측정 · 서버가 계산 · 앱은 렌더**(요금 근거를 조작 가능한 기기에 두지 않음) · 텔레매틱스 실패 4모드 · **지연 3단 상태**(집계중/임시-변동명시/최종) · ⭐**구간별 거리 × 구간별 단가**(구간1/구간2/구간 외) → 클라 계산 원천 불가 · quote 수명 · ⭐**반납 = 결제와 같은 등급의 one-way door**(멱등키+ack+optimistic 금지) · 압박방어 5
- [x] M4. 🔐 **차량 제어(스마트키)** — ✅ **초심층 완료**(2026-08-03) → `lessons/sd-m4-m6-control-search-deep.md`. ⭐**목표상태형 명령 코드로 검증**(Lock/Unlock 별도, 토글 없음) · ⭐**명령 단위 멱등성 3분류**(상태지정=멱등 / 단방향=멱등 / **경적=비멱등→자동재시도 ⛔**) · 이중경로 폴백 사다리 4단 + 숨은비용 · BLE 생명주기 5단 + **단계별 에러타입 분리** · ⭐⭐**전송 성공 ≠ 수행 완료**(write ack와 차량 응답이 별개 콜백) · 프로토콜 계층 직접 구현 이유 · 예약 바인딩 보안 · 단계 퍼널 관측 · 압박방어 4
- [x] M5. 💰 **요금 계산·정산 정합성** — ✅ M3와 통합 완료 → 위 `sd-m3-m5-telemetry-fare-deep.md` (서버 단일 계산 · breakdown 서버주도 렌더 · policyVersion · quote TTL)
- [x] M6. 🔎 **주변 검색 + 지도↔리스트** — ✅ 완료(2026-08-03, 위 파일에 통합). Repository SSOT · **typeahead 4단 방어**(distinct ≠ debounce 구분) · 정렬/필터 소유권 · 🔬 실무 대조: **debounce가 검색화면 중 1곳에만**(1초) + 1곳은 주석처리 → 공통 오퍼레이터 추출 제안

**학습 순서:** ~~`M2 → M1 → M3 → M5 → M4 → M6`~~ ✅ **M 트랙 6/6 초심층 완주**(2026-08-03) → 남은 것은 하나뿐: ⭐**능동 완주**(문서 없이 소리내어)

**🔬 실무 코드 그라운딩 (2026-08-03):** 사내 앱·웹 레포를 직접 읽어 **12건 패턴 대조** 완료. 익명화 규약 적용(식별자·경로·와이어 프로토콜 제외). 최대 소득 3개:
```
① 내 지도 문제는 클러스터링이 아니라 "2단 폴리곤 구역 판정"이었다 → 면접 오프닝 차별화
② 결제는 웹뷰 + 네이티브 브릿지(카드 OCR) 구조 → 웹뷰 결제 방어가 내 최강 답변
③ BLE 스마트키 = 3단 타임아웃 분리 + 예약번호 바인딩 핸드셰이크 + 직접 만든 프레이밍/체크섬
```

**모빌리티가 컨텐츠 앱과 다른 3가지 (한 줄 무기):**
```
① optimistic 금지 구역(결제·차량제어)이 존재한다   ② 배터리가 1급 제약(GPS=최대 소비원)
③ 앱 배포가 비가역 → 임계 수치(줌경계·TTL·주기)는 서버 설정값으로
```

### 📱 모바일 특화 체크리스트 (자발적으로 짚으면 depth 신호)
```
Offline-First   로컬DB=진실, 액션 로컬큐 적재→복구시 배치sync, optimistic update, Idempotency-Key
Sync/충돌       LWW(저가치 좋아요) < 충돌UI < OT(협업편집·서버권위) < CRDT(오프라인 멀티디바이스, 자동수렴)
                ⭐ 충돌 발생조건 = 협업 아님. `오프라인 × replica≥2`. 혼자 아이폰+아이패드도 충돌.
                ⭐ 수렴 ≠ 유실없음. LWW도 CRDT지만 정보를 버리며 수렴 / OR-Set·RGA는 보존하며 수렴.
                ⭐ 기기 wall clock 불신 → Lamport/HLC + replicaId 타이브레이크. append-only는 공짜 CRDT.
