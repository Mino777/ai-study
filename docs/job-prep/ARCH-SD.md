...
- [ ] M3. 📍 **주행 트립 트래킹** — 터널/지하 오프라인 버퍼 · **append-only = 공짜 CRDT**(B4 연결) · GPS 주기 적응 · iOS 백그라운드/강제종료 복구 · 요금 정합성
- [ ] M4. 🔐 **차량 제어(스마트키)** — BLE+서버 **이중 경로** · **목표상태형 명령(토글 ⛔)** · ack 필수 · 키 프로비저닝·replay 방지 · 실패 사다리
- [ ] M5. 💰 **요금 계산·정산 정합성** — 서버 단일 계산 · **breakdown 서버주도 렌더** · policyVersion · quote TTL (앱 배포=비가역이므로)
- [ ] M6. 🔎 **주변 검색 + 지도↔리스트** — 하나의 Repository SSOT (b2 근육 재사용)

**학습 순서 권장:** ~~`M2 → M1`~~ ✅ 초심층 완료 → **다음: M3 → M5 → M4 → M6** (요약은 뱅크에 있음, 초심층 확장 대기)
