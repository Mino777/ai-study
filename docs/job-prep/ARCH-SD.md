...
- [ ] M1. 🗺️ **실시간 차량 위치 지도** (도메인 최빈출) — bbox+zoom 쿼리 · **클러스터링 하이브리드**(낮은줌=서버집계/높은줌=클라) · **마커 diff 갱신 = 셀 재사용과 같은 문제** · 갱신주기 적응
- [ ] M2. 💳 **예약 → 결제** (⭐최고난도·최우선) — **상태머신**(held+TTL→paying→paid) · **이중결제 4중 방어** · **reconcile 루틴**(앱 죽어도 복구) · PG 웹뷰 이탈 · 보상 트랜잭션 · **⛔optimistic 금지 구역**
...
**학습 순서 권장:** `M2 → M1 → M3 → M5 → M4 → M6` — M2·M1이 "모빌리티 앱 만들어봤다"를 가장 빠르게 증명.
