...
- [ ] M4. 🔐 **차량 제어(스마트키)** — BLE+서버 **이중 경로** · **목표상태형 명령(토글 ⛔)** · ack 필수 · 키 프로비저닝·replay 방지 · 실패 사다리
- [x] M5. 💰 **요금 계산·정산 정합성** — ✅ M3와 통합 완료 → 위 `sd-m3-m5-telemetry-fare-deep.md` (서버 단일 계산 · breakdown 서버주도 렌더 · policyVersion · quote TTL)
- [ ] M6. 🔎 **주변 검색 + 지도↔리스트** — 하나의 Repository SSOT (b2 근육 재사용)

**학습 순서 권장:** ~~`M2 → M1 → M3 → M5`~~ ✅ 초심층 완료 → **다음: M4(차량제어) → M6(검색)** · 그리고 ⭐**능동 완주**(문서 없이 소리내어)
