...
const FDE_DATA: ResumeData = {
  ...
  work: [
    {
      ...
      highlights: [
        ...
        "**50+ 딥링크 경로** 설계 및 유지보수 — 쿠폰·예약·그린패스·알림톡 등 CRM/전환 경로 전반을 커버. 마케팅/CRM 파트와 직접 소통하며 비즈니스 요구사항을 기술로 해결",
        "Braze Geofence 위치 기반 푸시, Amplitude/Firebase GA4 트래킹, Airbridge 디퍼드 딥링크 등 **마케팅/분석 SDK 5종** 직접 연동 — 데이터 기반 의사결정 인프라 구축",
        "마케팅 부서가 CSV로 관리하는 이벤트 데이터와 앱 코드 간 불일치를 현장에서 발견 → Python + Shell Script로 타입 안전 Tracking API 자동 생성 모듈 개발, **컴파일 타임 100% 차단** — 일회성 수정이 아닌 구조적 해결",
        "신분증 OCR 프레임워크 교체 시 단순 교체가 아닌 **검출 실패율 데이터 수집 기반**까지 함께 구축 — 품질을 정량적으로 모니터링할 수 있는 구조로 전환",
        "ISMS-P 보안 취약점 대응 — 카드 정보 마스킹, 세션 재사용 차단, SMS 인증 우회 수정, OS 변조 탐지 우회 차단",
    ...
    {
      company: "마이다스에이치앤티", role: "iOS Developer", period: "2021. 03 — 2021. 11",
      summary: "KPNP(태권도 대회 영상 솔루션) 및 베베미테(스마트 홈 IoT) **iOS 1인 개발**.",
      highlights: [
        "KPNP(태권도 대회 영상 솔루션) iOS 1인 개발 — 프로젝트 생성 → **App Store 출시 2개월** 완료",
        "베베미테(스마트 홈 IoT) — Core Bluetooth + AWS AppSync IoT 연동, 서버팀과 협의하여 API 문서화 도입",
      ],
  ...
  projects: [
    ...
    { ...PROJECTS_IOS[1], tagline: "1인 4-Agent 동시 관제 · 3개 플랫폼 병렬 개발",
      bullets: [
        "1인이 4개 AI 에이전트를 동시에 운용하여 iOS/Android/Server를 병렬 개발 — API 명세 문서를 유일한 기준점으로 설정하고 **AI가 벗어나지 못하도록 Harness 적용**",
        "AI 출력물을 2단계 자동 검증(명세 대조 → 크로스 플랫폼 동기화 확인). 20라운드 중 초기 2건 실패 → 이후 **0건** — AI를 실전에 배포할 때 품질을 구조적으로 보장하는 방법의 실증",
        "작업 완료마다 회고+솔루션을 자동 생성, 다음 작업이 이를 입력으로 시작하는 **Compound** 구조 — 고객사 프로젝트를 반복 운영할수록 품질이 자동으로 올라가는 구조",
      ],
    },
    { ...PROJECTS_IOS[2],
      bullets: [
        "13개 AI 에이전트가 서로 논쟁하며 결론을 도출하는 파이프라인 — 입력 검증, 응답 모순 자동 차단, 실패 시 다른 AI 모델로 자동 전환하는 **Harness** 설계",
        "각 에이전트가 자기 전문 영역의 데이터만 보도록 격리(**Context**) — 영역 밖 정보 노출 시 환각 증가를 운영 데이터로 확인",
        "매 사이클 회고의 '다음 할 일'이 다음 세션의 입력이 되는 **Compound** 복리 구조",
      ],
    },
    { ...PROJECTS_IOS[3],
      bullets: [
        "25라운드 안전성 스프린트 — MoneyFlow의 패턴을 그대로 이식해 첫 라운드부터 높은 baseline 출발 (**재사용 가능한 프레임워크**의 실증)",
        "DB 보안 규칙 자동 감사가 사람 수동 검토보다 누락 1건 더 발견 — **자동화 게이트 > 수동 검토** 입증",
      ],
    },
  ],
  education: EDUCATION,
  extras: [
    {
      ...
      paragraphs: [
        "**문제**: MoneyFlow에서 13개 AI 에이전트를 운영하며, AI가 \"강력 매수\"라고 하면서 신뢰도는 30%인 모순된 응답을 내놓거나, 특정 시간대에 Claude API가 429 에러를 반복하며 서비스가 중단되는 문제가 발생했습니다.",
        "**해결**: 5단계 품질 검증 체계를 설계했습니다 — ① 입력 검증(타입/범위/null 체크) → ② 출력 검증(응답 내부 모순 자동 탐지) → ③ Bounded Retry(2회 실패 시 다른 AI 모델로 자동 전환) → ④ Conditional Routing(작업 복잡도에 따라 모델 자동 분배) → ⑤ LLM-as-a-Judge(별도 AI가 최종 출력 4차원 채점).",
        "**결과**: 이 패턴을 TypeScript(MoneyFlow) → Kotlin(Aidy 서버) → 3개 프로젝트에 이식. 언어와 도메인이 바뀌어도 프레임워크 자체는 재사용 가능함을 실증했습니다. Aidy에서 20라운드 운영 중 품질 검증 실패는 초기 2건 이후 **0건**입니다.",
        "**FDE 적용**: 고객사에 ALF를 배포할 때, AI 응답의 품질을 \"사람이 매번 확인\"하는 건 스케일 불가능합니다. 제가 만든 Harness 프레임워크는 이 문제를 구조적으로 해결합니다 — 입력 검증, 출력 검증, 폴백, 자동 채점을 레시피처럼 적용할 수 있습니다.",
      ],
    },
    {
      title: "② 현장 사용자를 만나 비즈니스 문제를 발견한 경험",
      subtitle: "마케팅팀의 CSV 파일이 앱을 망가뜨리고 있었다",
      paragraphs: [
        "**맥락**: 그린카에서 Braze/Amplitude/Airbridge 등 5종의 마케팅 SDK를 연동하면서, 마케팅/CRM 파트와 직접 협업하는 과정에서 문제를 발견했습니다.",
        "**발견**: 마케팅 현업 부서는 이벤트 트래킹 데이터를 CSV 파일로 관리하고 있었습니다. 이벤트명이나 파라미터가 변경되면 CSV를 수정하고 개발팀에 전달하는데, 이 과정에서 **이벤트명 오타, 파라미터 불일치, 삭제된 이벤트 잔존** 등의 문제가 런타임에서야 발견되었습니다. 잘못된 이벤트 트래킹은 마케팅 데이터 전체의 신뢰도를 떨어뜨렸고, 원인을 찾는 데 매번 수 시간이 소요되었습니다.",
        "**행동**: 이 문제를 \"커뮤니케이션을 더 잘하자\"로 풀지 않았습니다. Python + Shell Script로 Run Script 환경에서 CSV를 자동 파싱하여 타입 안전한 Tracking API 코드를 생성하는 모듈을 만들었습니다. 마케팅팀이 CSV를 수정하면 빌드 시점에 자동으로 반영되고, 불일치가 있으면 **컴파일이 실패**합니다.",
        "**결과**: 이벤트 불일치로 인한 데이터 오염이 **0건**으로 떨어졌습니다. 마케팅팀은 CSV만 관리하면 되고, 개발팀은 \"이벤트명이 맞는지\" 수동 확인할 필요가 없어졌습니다. 일회성 수정이 아닌, **구조적으로 문제가 재발하지 않는 시스템**을 만든 것입니다.",
        "**FDE 적용**: FDE의 핵심은 \"고객사별 일회성 커스텀이 아니라, 현장에서 검증된 솔루션을 스케일러블하게 만드는 것\"입니다. 저는 그린카에서 정확히 이 패턴으로 일했습니다 — 현장 문제 발견 → 구조적 해결 → 재사용 가능한 모듈화.",
      ],
    },
  ],
};
