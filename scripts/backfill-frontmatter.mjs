...
function inferApplicableTo(filename, content) {
  const aidyTerms = ["aidy", "architect-cli", "watch-workers", "tmux-setup", "autoceo"];
  const hasAidy = aidyTerms.some(
    (t) => filename.includes(t) || content.includes(t)
  );
  return hasAidy ? '["aidy", "any"]' : '["any"]';
}
...
function processFile(filePath, filename) {
  ...
  if (hasLastVerified && hasApplicableTo) {
    skippedFiles++;
    return;
  }
  ...
  if (!hasApplicableTo) {
    ...
    // last_verified 뒤에 삽입하거나, confidence 뒤에 삽입
    if (!hasLastVerified && date) {
