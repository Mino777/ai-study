...
async function generateMDX(topic, manifest) {
  ...
  const contextNote = topic.category === "ios-ai"
    ...
    : "\n대상 독자: iOS 시니어 개발자(Swift 전문)가 AI 엔지니어링을 학습하는 맥락. 코드 예제가 필요하면 Swift 우선, 불가능하면 Python. TypeScript는 프론트엔드 카테고리에서만.";

  const prompt = `다음 주제에 대한 깊이 있는 기술 분석 글을 작성하세요.
인사말이나 자기소개 없이 바로 핵심부터 시작하세요.
...
## 메타데이터 (본문 전에 반드시 출력)
- 첫 번째 줄: TITLE: 정제된 제목 (핵심 키워드 + 부제. 과장 수치 금지. 구어체 금지)
- 두 번째 줄: DESC: 한 줄 설명 (이 글을 읽으면 뭘 할 수 있게 되는지)
- 세 번째 줄: TAGS: 영문 태그 3~6개 (쉼표 구분, 한국어 금지)

## 글쓰기 원칙 — AI slop 방지

**절대 하지 말 것:**
- "~의 중요성은 아무리 강조해도 지나치지 않습니다" 같은 빈 수식어
- "~할 수 있습니다", "~가 기대됩니다" 같은 추측성 서술
- 교과서 목차를 나열하듯 개념을 평면적으로 설명
- 모든 항목을 똑같은 깊이로 다루기 (중요한 것에 집중)
- 실행 불가능한 의사코드를 특정 언어(TypeScript 등)로 포장

**반드시 할 것:**
- 첫 문단에서 "이 주제를 왜 지금 다뤄야 하는지" 긴장감 있게 시작
- 하나의 핵심 인사이트를 깊이 파고들기 (넓고 얕은 것보다 좁고 깊은 것이 낫다)
- 트레이드오프와 한계를 솔직하게 다루기 ("이 접근은 X 상황에서는 오히려 해롭다")
- 비교 테이블로 선택 기준을 명확히 제시
- "그래서 뭘 해야 하는데?" 라는 질문에 답하기

## 코드 규칙
- **코드가 필요할 때**: Swift 우선. Swift가 부적합하면 Python. TypeScript는 frontend-ai 카테고리에서만.
- **실행 불가능한 의사코드는 코드 블록으로 쓰지 마세요.** 워크플로우는 Mermaid 다이어그램, 개념은 테이블이나 서술로.
- Mermaid 다이어그램 1개 이상 필수 (\`\`\`mermaid 코드 블록)
