...
요구사항:
- 첫 번째 줄에 반드시 TITLE: 정제된 제목 을 작성하세요 (예: "TITLE: Compound Engineering 기초 — 복리형 AI 개발 방법론"). 사용자 요청을 기술 블로그에 어울리는 전문적인 제목으로 변환하세요. 구어체/요청문("~하고싶어", "~에 대해" 등)은 제거하고, 핵심 키워드 + 부제 형식으로 작성. ⚠️ 제목에 과장된 수치("98% 절감", "100만 스케일", "10x 생산성") 금지 — 간결하고 정확하게.
...
**품질 가드 (매우 중요):**
1. **실측 데이터 필수**: "~할 수 있다", "~가 기대된다" 같은 추측 금지. 구체적 수치는 출처가 있어야 함. 출처 없으면 반드시 "이론적으로" 명시하고, 가능하면 실제 프로젝트(ai-study/moneyflow/tarosaju/aidy) 적용 사례로 뒷받침.
2. **프로젝트 경험 연결 필수**: 단순 이론이 아니라, ai-study/moneyflow/tarosaju/aidy 중 하나의 프로젝트에서 실제로 적용할 수 있는 시나리오를 포함. "이론적으로 가능하다"가 아닌 "이 프로젝트의 이 부분에 적용하면" 수준으로 구체화.
3. **중복 체크**: 기존 엔트리 목록을 보고 이미 다루고 있는 주제를 절대 반복하지 말 것.
4. **Slug 규칙**: 최종 slug는 영문 kebab-case만. 40자 이내. SEO용 키워드 나열 금지.
5. **외부 권위 자료 우선**:
   - 프로젝트 자체 사례만으로 엔트리를 채우지 말 것
   - 반드시 외부 권위 자료를 먼저 제시하고, 프로젝트 사례는 "우리 프로젝트 적용" 섹션에서 보충
   - 우선순위: ①빅테크 엔지니어링 블로그(Google, Meta, Anthropic, OpenAI) ②학술 논문/컨퍼런스(NeurIPS, ICML, ACL) ③업계 권위자 블로그(Martin Fowler, Simon Willison, Andrej Karpathy) ④공식 문서(Apple, Google Cloud, AWS)
   - 각 외부 자료는 제목+출처+핵심 인사이트 1줄로 인라인 인용
   - 자료가 없는 주장은 "경험적 관찰" 또는 "이론적으로"로 명시
...
4. **Mermaid 5대 함정 (매우 중요)**:
   a. 괄호 안 특수문자: subgraph/node 라벨에 ( ) [ ] 있으면 "..." 감싸기. 예: F{"Check Rate (Firestore)"}
   b. <br/> 금지: Mermaid는 줄바꿈을 \\n 또는 · 사용
   c. 콜론 : 뒤 공백: node 라벨에 콜론 있으면 "..."로 감싸기. 예: D["Deploy: Production"]
   d. subgraph/node ID 충돌 금지: 같은 이름 사용 금지
   e. 유니코드 화살표(→) 금지: Mermaid는 --> 사용. 예: A --> B (← → ⇒ 금지)
   f. Mermaid 라벨 내에 코드 신택스(역슬래시나 특수문자) 직접 사용 금지`;
