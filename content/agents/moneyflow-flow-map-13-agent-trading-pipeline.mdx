...
quiz:
  - question: MoneyFlow에서 분석 에이전트가 실패하면 어떤 패턴으로 대응하나?
    ...
    answer: 1
    explanation: >-
      Bounded Retry: Claude 2회 실패 → Gemini 전환 → GPT 폴백. 무한 재시도는 비용 폭증 + 사용자 대기.
      같은 인터페이스(LSP)라 모델 교체가 투명.
  - question: 13개 에이전트가 각각 한 가지 분석만 담당하는 설계 원칙은?
    ...
    answer: 1
    explanation: >-
      SOLID의 SRP를 에이전트에 적용. 하나의 에이전트가 기술분석+펀더멘털+뉴스를 다 하면 예측 불가 행동 + 비용 폭증. 13개로
      분리하면 개별 테스트·교체·모니터링 가능.
  - question: 분석 비용 제한을 해제(#163)한 이유는?
    ...
    answer: 1
    explanation: >-
      13개 에이전트 중 비용 제한이 걸린 에이전트는 토큰을 아끼려고 분석을 축약 → 결과 품질 하락. '풀스로틀'로 전환하여 품질 우선
      + 비용은 Prompt Caching으로 최적화.
---
