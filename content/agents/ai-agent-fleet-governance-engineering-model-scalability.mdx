...
quiz:
  - question: 본문에서 AI 에이전트 플릿을 '엔지니어링 조직'처럼 관리해야 한다고 주장하는 가장 핵심적인 이유는 무엇인가요?
    ...
    answer: 2
    explanation: >-
      정답은 3번입니다. 본문에서는 에이전트 수가 많아지면 추적이 불가능해지는 문제를 지적하며, '엔지니어링 조직' 모델이 수십 년간
      검증된 관리 프레임워크로서 에이전트 플릿에 가시성, 책임성, 확장성, 안정성을 부여하는 핵심 해결책이라고 설명합니다. 1, 2번은
      거버넌스의 일부 요소(비용 측정, Prompt-as-Code)이지만 모델을 도입하는 가장 근본적인 이유는 아닙니다. 4번은
      '차용'하는 개념을 '동일하게 만든다'고 오해한 것입니다.
  - question: 에이전트 생명주기(Agent Lifecycle)에서 'Deprecated' 상태의 에이전트는 어떤 역할을 수행하나요?
    ...
    answer: 3
    explanation: >-
      정답은 4번입니다. 'Deprecated' 상태는 새로운 버전이 'Active' 상태로 배포되었을 때, 기존 버전의 에이전트가
      수행하는 역할입니다. 새로운 요청은 새 버전으로 가지만, 기존에 처리하던 요청은 마무리지어 서비스 중단 없이 안전하게
      전환(graceful transition)하는 것을 돕습니다. 1번은 'Staging', 2번은 'Retired', 3번은
      'Experimental' 상태에 대한 설명입니다.
  - question: >-
      본문에 따르면, 개선된 새 버전의 에이전트가 기존 'Active' 에이전트를 대체하기 위한 '승진 프로세스'에서 가장 중요한 검증
      단계는 무엇인가요?
    choices:
    ...
    answer: 1
    explanation: >-
      정답은 2번입니다. '승진 프로세스'의 핵심은 새로운 에이전트가 기존 에이전트보다 실제로 더 나은 성능을 내는지 '검증'하는
      것입니다. 본문의 CI/CD 파이프라인 다이어그램은 자동화된 Evals 통과 후, Canary/A/B 테스트를 통해 성능 우위가
      확인되어야 'Active'로 승격될 수 있음을 명확히 보여줍니다. 1번은 성능 측정을 위한 준비 단계, 3번은 행정적인 기록, 4번은
      프로세스의 시작 단계일 뿐, 최종 승진을 결정하는 핵심 검증 단계는 아닙니다.
---
