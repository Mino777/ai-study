...
status: complete
description: >-
  에이전트 간 통신의 3가지 패턴(Direct, Async, Shared State)과 실전 선택 기준. ai-study messages/
  파일 기반 큐, moneyflow 13-agent 파이프라인, aidy architect-worker 사례.
type: entry
quiz:
  - question: 에이전트 간 직접 호출(Direct) 패턴이 가장 적합한 상황은?
    ...
    answer: 1
    explanation: >-
      Direct Call은 낮은 지연성이 특징. Bull-Bear 토론처럼 한 에이전트의 결과가 다음 에이전트의 입력이 되는 강한
      의존성이 있을 때 가장 효율적.
  - question: 파일 기반 메시지 큐(messages/)의 장점은?
    ...
    answer: 1
    explanation: >-
      파일 기반 큐는 외부 인프라(Redis, RabbitMQ) 없이 순수 파일 시스템으로 메시지 저장. 모든 메시지가 git에 남아 감사
      추적 가능하고, 에러 발생 시 재생산 용이.
  - question: moneyflow의 13-agent 파이프라인에서 에이전트 간 통신에 사용되는 주 메커니즘은?
    ...
    answer: 2
    explanation: >-
      MoneyFlow는 각 에이전트가 JSON Schema로 구조화된 결과를 산출하고, 다음 에이전트가 그 JSON을 입력으로 받음.
      빠른 응답과 명확한 인터페이스가 핵심.
---
