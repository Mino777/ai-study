...
applicable_to: ["any"]
connections: []
description: "MLflow 3.9.0의 AI Assistant에서 발생한 `CVE-2026-2611` 취약점을 분석하며, 루프백 인터페이스가 신뢰 경계가 될 수 없다는 보안 원칙을 강조합니다. 로컬에서 실행되는 AI 에이전트 및 서비스가 어떻게 외부 공격에 노출될 수 있는지 설명합니"
generated_by: gemini
quiz:
...
CVE-2026-2611의 공격 체인은 각 단계가 합법적인 기능을 사용하지만, 그 조합이 치명적인 결과를 낳는다는 점에서 교훈적이다. National Vulnerability Database(NVD)는 이 취약점에 CVSS 9.6 (CRITICAL) 등급을 부여했다.
...
**수정된 코드 (MLflow 3.10.0, [PR #20832](https://github.com/mlflow/mlflow/pull/20832) 기반):**
```python
# security_utils.py
API_PATH_PREFIX = "/api/"
...
---
이 엔트리는 Blake Crosley의 [Loopback Is Not a Trust Boundary: CVE-2026-2611](https://blakecrosley.com/blog/cve-2026-2611-loopback-is-not-a-trust-boundary/)을 정독하고 핵심을 추출한 것이다.