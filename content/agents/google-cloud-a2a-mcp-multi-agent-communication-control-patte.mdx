...
graph TD
    subgraph A2A_Pattern ["A2A Pattern · Event-Driven"]
        ...
        IngestionAgent -- "image_analysis_needed 토픽 발행" --> PubSub["Google Cloud Pub/Sub"]
        PubSub -- "Message Delivered" --> VisionAgent["VisionAnalysisAgent"]
        VisionAgent -- "text_generation_needed 토픽 발행" --> PubSub
        PubSub -- "Message Delivered" --> TextAgent["TextGenerationAgent"]
        TextAgent -- "final_result 토픽 발행" --> PubSub
...
graph TD
    subgraph MCP_Pattern ["MCP Pattern · Orchestrated"]
        UserRequest["User Request"] --> MCP["Cloud Workflows · Orchestrator"]
        MCP -- "Step 1" --> FlightAgent["FlightSearchAgent"]
        FlightAgent -- "Flight Data" --> MCP
        MCP -- "Step 2" --> HotelAgent["HotelSearchAgent"]
        HotelAgent -- "Hotel Data" --> MCP
        MCP -- "Step 3" --> SynthesizerAgent["ItinerarySynthesizer"]
