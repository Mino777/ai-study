...
applicable_to: ["any"]
connections: []
description: "이 글은 에이전트가 필요한 정보를 찾는 과정을 '런타임 문제'로 정의하고, 기존의 `grep` 방식과 최신 `벡터 검색` (RAG, 임베딩) 기술을 비교합니다. 효율적인 정보 검색이 에이전트의 성능과 자율성에 결정적인 영향을 미친다는 점을 강조하며, 다양한 검색 기술의"
generated_by: gemini
quiz:
