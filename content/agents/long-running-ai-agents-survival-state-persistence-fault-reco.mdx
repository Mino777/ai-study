...
quiz:
  - question: 장기 실행 AI 에이전트 설계에서 '상태(State)를 실행(Execution)과 분리'하는 가장 근본적인 이유는 무엇입니까?
    choices:
      - >-
        에이전트 실행 프로세스는 언제든 중단될 수 있으므로, 작업 진행 상황을 외부 영구 저장소에 보관하여 장애 발생 시에도 작업을
        복구하고 재개할 수 있도록 하기 위함이다.
    ...
    answer: 0
    explanation: >-
      정답입니다. 본문은 장기 실행 에이전트가 '실패는 필연적'이라는 가정 하에 설계되어야 한다고 강조합니다. 실행(프로세스)은 일회용으로
      취급하고, 상태(진행 상황)는 외부에 영구 저장함으로써 프로세스가 예기치 않게 종료되더라도 마지막 작업 지점부터 안전하게 이어갈 수
      있습니다. 이것이 바로 상태와 실행을 분리하는 핵심적인 이유인 '장애 복구'와 '신뢰성' 확보입니다. 나머지 선택지들은 일반적인
      소프트웨어 설계 목표일 수는 있으나, 본문에서 강조하는 장기 실행 에이전트의 핵심 생존 전략과는 거리가 습니다.
  - question: >-
      본문에서 '작업 체크포인팅(Task Checkpointing)'을 구현할 때, 각 단위 작업이 반드시 가져야 할 가장 중요한 특성으로
      언급된 것은 무엇입니까?
    choices:
    ...
    answer: 1
    explanation: >-
      정답입니다. 본문은 '체크포인팅의 핵심은 멱등성(Idempotency)'이라고 명시하고 있습니다. 에이전트가 실패 후 특정 작업을
      재시도할 때, 해당 작업이 이미 부분적으로 또는 전체적으로 완료되었을 수 있습니다. 멱등성이 보장되면, 동일한 작업을 여러 번
      실행해도 시스템에 부작용을 일으키지 않고 항상 일관된 최종 결과를 만들어내므로 안전한 복구가 가능합니다. 예를 들어 '파일 A 생성'
      작업은 파일이 이미 존재하더라도 오류 없이 성공으로 처리되어야 합니다. 원자성은 오히려 단기 실행 트랜잭션의 특징이며, 비동기성이나
      고유성은 멱등성과는 다른 개념입니다.
  - question: >-
      제공된 Python 코드 예제와 아키텍처 다이어그램을 참고할 때, 에이전트가 처음 실행되어 'Executing step 2...'를
      처리하던 중 예기치 않게 프로세스가 강제 종료되었다고 가정합시다. 이 에이전트가 다시 시작될 때 가장 먼저 수행하는 동작은
      무엇입니까?
    choices:
      - >-
        지정된 ID의 상태 파일(`_state.json`)을 찾아 마지막 체크포인트를 메모리로 로드하고, 중단되었던 2단계부터 작업을
        재개한다.
      - >-
        상태 파일이 비정상적으로 종료되었을 가능성이 있으므로, 안전을 위해 기존 상태 파일을 무시하고 0단계부터 모든 작업을 새로
        시작한다.
    ...
    answer: 0
    explanation: >-
      정답입니다. 본문의 아키텍처와 코드의 `_load_state` 메서드는 에이전트가 시작될 때 가장 먼저 외부 저장소(여기서는 JSON
      파일)에서 마지막 상태를 불러오도록 설계되었습니다. 코드 예제에서 각 단계가 성공적으로 완료된 후에만 상태(`next_step`)가
      업데이트되고 저장됩니다. 따라서 2단계 실행 중 충돌이 발생했다면, 상태 파일에는 `next_step`이 여전히 2로 기록되어 있을
      것입니다. 에이전트는 이 상태를 로드하여 중단되었던 지점인 2단계부터 자동으로 실행을 재개합니다. 이는 '자동 복구 및
      재개(Automatic Recovery & Resumption)' 패턴의 핵심 동작입니다.
---
