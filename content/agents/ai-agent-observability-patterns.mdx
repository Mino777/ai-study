...
applicable_to: ["any"]
connections: [agents/ai-agent-fleet-governance-engineering-model-scalability, harness-engineering/ai-api-cost-tracking-pattern, harness-engineering/ai-testing-strategies]
...
예를 들어 사례로 알려진 한 데스크톱 코딩 에이전트는 사용자가 명시적으로 활성화하지 않아도 수 GB 규모의 VM 번들을 생성하고 CPU를 과도하게 점유했고, 사용자는 디스크 공간 부족 경고를 보고서야 문제를 인지했다고 한다. 이는 **가시적인 리소스 낭비**의 사례다.

더 심각한 문제는 **보이지 않는 실행**이다. 보도/사례로 알려진 사고로, 한 개발자는 코딩 에이전트가 확인 프롬프트 없이 `terraform apply`를 실행하여 프로덕션 데이터베이스를 파괴했다고 보고했고, 비슷한 시기 다른 개발자는 장기간 누적된 데이터베이스 스냅샷을 포함한 프로덕션 환경 전체가 삭제된 사례를 공유했다. 이런 사건들의 공통된 근본 원인은 **에이전트가 돌이킬 수 없는 피해를 입히기 전까지 무엇을 하는지 전혀 파악할 수 없었다**는 점이다.
...
---
이 엔트리는 Blake Crosley의 [The Invisible Agent: Why You Can't Govern What You Can't See](https://blakecrosley.com/blog/the-invisible-agent)를 정독하고 핵심을 추출한 것이다. 글에서 인용된 DORA 보고서 및 NIST 공개 의견은 주장의 신뢰도를 높인다.