...
class ToolExecutor:
    ...
    def execute_tool(self, tool_call: Dict) -> Any:
        ...
        arguments = eval(tool_call["function"]["arguments"]) # WARNING: Use proper JSON parsing in real app
...
class AutonomousAgent:
    ...
    def _think_and_act(self) -> str:
        ...
        # LLM에 현재까지의 대화 내역(기억)과 사용 가능한 툴 스키마를 전달
        ...
        if "tool_calls" in llm_response and llm_response["tool_calls"]:
            for tool_call in llm_response["tool_calls"]:
                try:
                    ...
                    print(f"  [Agent {self.name}] Tool '{tool_call['function']['name']}' executed. Output: {tool_output}")
                    
                    # 툴 실행 결과를 다시 LLM에 전달하여 다음 행동 추론
                    return self._think_and_act() # 재귀 호출하여 툴 실행 후 LLM이 다시 생각하도록 함
                    ...
                    return f"죄송합니다. 작업 중 오류가 발생했습니다: {error_message}"
        # 툴 호출 없이 LLM이 직접 응답하는 경우
        else:
            agent_response = llm_response.get("content", "처리할 수 없습니다.")
            self.memory.append({"role": "assistant", "content": agent_response})
            print(f"  [Agent {self.name}] Responding: '{agent_response}'")
...
if __name__ == "__main__":
    ...
    print(f"Final Agent Response: {response1}")
    
    print("\n--- 2단계: 일반 질문 ---")
    response2 = travel_agent.run("오늘 점심 뭐 먹지?")
    print(f"Final Agent Response: {response2}")
    
    print("\n--- 3단계: 알림 요청 ---")
    response3 = travel_agent.run("나한테 '회의 10분 전'이라고 알림 보내줘 (내 ID는 user123)")
    print(f"Final Agent Response: {response3}")
