...
connections:
  ...
  - agents/solid-principles-for-agent-design
...
quiz:
  - question: 자율 에이전트의 Sense-Think-Act 루프 중 'Think' 단계의 핵심 역할은 무엇인가요?
    ...
    answer: 1
    explanation: >-
      Think 단계는 LLM을 사용하여 현재 상황을 분석하고 최적의 행동 계획을 수립하는 추론(Reasoning) 단계입니다.
      Sense는 정보 수집, Act는 행동 실행, Observability는 모니터링을 담당하므로, Think 단계가 추론과 계획의
      중추입니다.
  - question: 파이썬 예제의 ToolExecutor에서 execute_tool() 메서드가 하는 역할은 무엇인가요?
    ...
    answer: 1
    explanation: >-
      ToolExecutor는 LLM이 선택한 도구의 이름과 인자(arguments)를 받아, self.available_tools
      딕셔너리에서 해당 함수를 찾아 실행하는 '행동 실행' 계층입니다. 이는 Act 단계의 구현입니다.
  - question: 본 글에서 설명하는 2026년 트렌드 'Human-in-the-Loop Collaboration'의 핵심 목적은 무엇인가요?
    ...
    answer: 1
    explanation: >-
      Human-in-the-Loop Collaboration은 에이전트가 복잡한 결정을 내리기 전에 인간의 검토나 승인을 요청하고,
      피드백을 통해 학습하며 점차 자율성을 높여가는 패턴입니다. 이는 AI의 자율성(효율성)과 인간의 통제(안전성) 사이의 균형을 핵심으로
      합니다.
---
