...
description: >-
  Anthropic이 공개한 멀티시간 자율 개발을 위한 3-Agent 하네스: Planning/Generation/Evaluation 분리,
  컨텍스트 리셋 전략, Few-shot Calibration 평가
category: agents
date: '2026-05-02'
tags:
  - anthropic
...
quiz:
  - question: Anthropic 3-Agent에서 컨텍스트 압축(compaction) 대신 리셋을 사용하는 이유는?
    ...
    answer: 1
    explanation: >-
      컨텍스트 압축은 모델이 '곧 한계에 도달할 것'을 인식하여 보수적으로 행동하게 만든다. 리셋 + 구조화된 아티팩트 전달이 더 깔끔한
      상태에서 작업을 이어갈 수 있게 해준다.
  - question: 생성 에이전트와 평가 에이전트를 분리하는 핵심 이유는?
    ...
    answer: 2
    explanation: >-
      모델은 자기 출력을 과대평가하는 경향이 있다(특히 주관적 작업). 별도 에이전트가 독립적으로 평가하면 이 편향을 깨뜨릴 수 있다.
      LLM-as-a-Judge의 핵심 원리.
  - question: 이 패턴에서 Frontend 디자인 평가의 4가지 메트릭은?
    ...
    answer: 1
    explanation: >-
      Design quality, Originality, Craft, Functionality. 주관적 영역에서도 구조화된 평가 기준을
      적용하는 것이 핵심.
---
