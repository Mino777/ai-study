...
status: complete
description: >-
  GBrain(기억) + OpenClaw(실행) + Hermes(학습/오케스트레이션)의 3계층 에이전트 스택 분석. 각 레이어의 역할 분담과
  Delegation 패턴을 정리하고, 내 프로젝트 적용 가능성을 탐색.
type: entry
quiz:
  - question: GBrain + OpenClaw + Hermes 스택에서 GBrain이 담당하는 핵심 역할은?
    ...
    answer: 1
    explanation: >-
      GBrain은 Garry Tan이 만든 에이전트 기억 시스템으로, 마크다운 파일 + Postgres/pgvector 하이브리드
      검색으로 지식 그래프를 구축한다. 에이전트들이 세션 초기화 후에도 과거 정보를 유지하는 '뇌' 역할.
  - question: Hermes가 OpenClaw에 작업을 위임하는 "Chief of Staff" 패턴의 핵심 동기는?
    ...
    answer: 1
    explanation: >-
      Hermes는 자기 개선 루프와 메모리에 강하지만, 로컬 환경 제어(bash, 파일 시스템, 브라우저)는 OpenClaw가 더
      강력하다. 전략적 사고는 Hermes가, 실행은 OpenClaw가 맡는 계층적 분업.
  - question: GBrain의 "Compiled Truth + Timeline" 패턴에서 위쪽과 아래쪽의 역할은?
    ...
    answer: 1
    explanation: >-
      마크다운 파일의 상단은 최신 종합 평가로 갱신되고, 하단은 날짜별 이벤트가 append-only로 쌓인다. 이렇게 하면 최신 판단과
      과거 근거를 한 파일에서 동시에 관리할 수 있다.
---
