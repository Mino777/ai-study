...
confidence: 2
connections: [agents/agent-architectures, agents/agent-compiler-dynamic-routing, agents/ai-agent-fleet-governance-engineering-model-scalability, context-engineering/hydrallm-multi-llm-orchestration]
status: draft
description: "Reflex 벤치마크가 측정한 45배 비용 차이를 출발점으로, UI 조작과 API 호출 사이의 선택 기준 · 폴백 패턴 · iOS App Intents 통합을 정리합니다."
type: entry
generated_by: gemini
quiz:
  - question: AI 에이전트의 'Computer Use' 방식이 '구조화된 API 호출' 방식보다 평균 45배 더 비싼 핵심적인 이유는 무엇입니까?
    ...
    answer: 2
    explanation: 정답입니다. Reflex 벤치마크는 vision agent가 53 단계에 걸쳐 약 55만 input 토큰을 소비한 반면 API agent는 8번의 호출로 1.2만 토큰을 사용했음을 보여줍니다. 모든 단계마다 스크린샷을 LLM에 입력해야 하므로 "보기 위한 비용"이 누적됩니다. 더 좋은 비전 모델은 단계당 오류율을 낮출 뿐, 필요한 스크린샷 개수를 줄이지 못합니다.
  - question: 본문에서 제안하는 'API-First, UI-Fallback' 전략을 채택했을 때 얻을 수 있는 가장 큰 이점은 무엇입니까?
    ...
    answer: 1
    explanation: 정답입니다. 이 패턴의 핵심은 통제 가능한 시스템에는 API를, 통제 불가능한 외부 SaaS/레거시에만 Computer Use를 쓰는 것입니다. Reflex 저자도 "applications you do not control" 즉 변경할 수 없는 시스템에서만 vision agent가 옳다고 명시합니다.
  - question: Reflex 벤치마크에서 vision agent가 처음에 작업을 완료하지 못한 이유는 무엇입니까?
    ...
    answer: 1
    explanation: 정답입니다. 벤치마크 보고서는 "vision agent가 렌더된 페이지에 대해 추론하면서, 페이지가 모든 항목을 보여주지 않는다는 신호를 받지 못했다"고 적습니다. 결국 14단계의 명시적 UI 워크스루 프롬프트를 추가해야 작업이 완료됐습니다 — 이는 vision agent가 "보이지 않는 상태(off-screen state)"를 추론하지 못한다는 본질적 한계를 보여줍니다.
