...
> 이 엔트리는 Blake Crosley의 [AI Agents Need Exploration Checkpoints](https://blakecrosley.com/blog/ai-agents-need-exploration-checkpoints)을 정독하고 핵심을 추출한 것이다.
...
논문의 실험 결과는 이 패턴의 유효성을 뒷받침한다.
- **편향 교정:** 목표 지향적 훈련(GRPO)은 오히려 에이전트의 탐색을 좁혔다 (Qwen3-4B의 ECC가 28.5% → 18.8%로 하락).
- **성능 향상:** 탐색을 인지하는 훈련을 받은 에이전트는 Explore-then-Act 구조에서 직접 실행보다 높은 성공률을 보였다 (Qwen3-4B 기준 77.2% → 79.5%).
...
> 이 엔트리는 Blake Crosley의 [AI Agents Need Exploration Checkpoints](https://www.latent.space/p/ai-agents-need-exploration-checkpoints)를 정독하고, 글이 인용한 핵심 논문인 Ziang Ye 등의 ["Look Before You Leap: A Universal Exploration-Exploitation Policy for Action-Based Language Models"](https://arxiv.org/abs/2405.09438)의 아이디어를 추출하여 재구성한 것이다.