...
이 엔트리는 Blake Crosley의 [Agents.txt Is Not Access Control](https://blakecrosley.com/blog/agents-txt-is-not-access-control/)을 정독하고 핵심을 추출한 것이다.
...
**2. 서버 미들웨어를 통한 크롤러 신원 검증**

Vercel Edge Middleware를 사용하여 `Googlebot`을 사칭하는 악성 봇을 차단하는 로직을 구현한다.
...
import { a, dns } from '@vercel/dns';

const GOOGLEBOT_HOSTNAMES = ['.googlebot.com', '.google.com'];

export async function middleware(request: NextRequest) {
  const userAgent = request.headers.get('user-agent') || '';
  const ip = request.ip;

  // Check if the user agent claims to be Googlebot
  if (userAgent.includes('Googlebot') && ip) {
    try {
      // 1. Perform a reverse DNS lookup on the request's IP address.
      const hostnames = await dns.reverse(ip);
      
      // 2. Verify that the hostname ends with .googlebot.com or .google.com.
      const isVerifiedGooglebot = hostnames.some(hostname => 
        GOOGLEBOT_HOSTNAMES.some(validSuffix => hostname.endsWith(validSuffix))
      );

      if (!isVerifiedGooglebot) {
        // If it's a fake Googlebot, block the request.
        return new NextResponse('Forbidden: IP address does not match Googlebot hostname.', { status: 403 });
      }

      // Optional: Forward DNS lookup for stronger verification.
      // const forwardIps = await a(hostnames[0]);
      // if (!forwardIps.includes(ip)) { ... }

    } catch (error) {
      // DNS lookup failed, treat as suspicious.
      console.warn('DNS verification failed for IP:', ip, error);
      return new NextResponse('Forbidden: DNS verification failed.', { status: 403 });
    }
  }
...
이 접근법을 통해 `tarosaju`는 `robots.txt`로 정책 '의도'를 명확히 표현하고, 서버 엣지에서 '실제' 검증을 수행하여 검색 엔진 최적화(SEO)의 이점은 누리면서 콘텐츠를 보호하는 이중 방어 체계를 구축할 수 있다.