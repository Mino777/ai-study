...
class AgentCompiler {
  ...
  // 시스템 프롬프트 생성
  private renderSystemPrompt(context: AgentContext): string {
    const promptParts = this.components
  ...
  // 도구 스키마 생성
  private renderToolSchemas(tools: AgentTool[]): string {
    return tools
      .map(t => `- **${t.name}**: ${t.description}`)
      .join('\n');
  }

  // 최종 컴파일: 시스템 프롬프트 + 도구 정보
  compile(context: AgentContext): { systemPrompt: string; tools: AgentTool[] } {
    ...
    // 도구 사용 지침 추가 (Premium 사용자만)
    let enhancedPrompt = systemPrompt;
    if (tools.length > 1) {
      enhancedPrompt += '\n\n## 사용 가능한 도구\n' + this.renderToolSchemas(tools);
    }

    return { systemPrompt: enhancedPrompt, tools };
  }
}
...
console.log('\nAvailable Tools:');
console.log(freeAgentSpec.tools.map(t => t.name).join(', '));
...
console.log('\nAvailable Tools:');
console.log(premiumAgentSpec.tools.map(t => t.name).join(', '));

// 에이전트 호출 예시 (실제 Claude API 호출)
async function callAgentWithCompiledConfig(context: AgentContext, userQuery: string) {
  const agentSpec = compiler.compile(context);
  
  // Claude API 호출 (의사 코드)
  const response = await anthropic.messages.create({
    model: 'claude-3-5-sonnet-20241022',
    max_tokens: 1024,
    system: agentSpec.systemPrompt,
    tools: agentSpec.tools.map(t => ({
      name: t.name,
...
// 실행
callAgentWithCompiledConfig(premiumUserContext, '내 데이터를 분석해줄 수 있어?')
