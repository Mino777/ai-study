...
quiz:
  - question: 본문에서 설명하는 에이전트 '컴파일' 전략의 핵심적인 특징으로 가장 적절한 것은 무엇입니까?
    ...
    answer: 2
    explanation: >-
      정답은 3번입니다. 본문은 '컴파일' 전략을 '런타임에 주어진 컨텍스트에 맞춰 최적의 설정(프롬프트, 도구 등)을 동적으로 생성하는
      접근 방식'이라고 정의합니다. TypeScript 예제와 고객 지원 챗봇 시나리오 모두 사용자 등급이라는 컨텍스트에 따라 다른
      프롬프트와 도구를 실시간으로 조합하는 것을 보여줍니다. 1번은 본문에서 '전통적인 의미가 아니다'라고 명시적으로 부정했습니다. 2번과
      4번은 정적인 설정 파일의 한계를 벗어나지 못하는 방식으로, '컴파일' 전략이 해결하려는 문제점에 해당합니다.
  - question: 본문이 지적한 기존 YAML 설정 방식의 한계점으로 가장 거리가 먼 것은 무엇입니까?
    ...
    answer: 3
    explanation: >-
      정답은 4번입니다. 본문 첫 문단에서 YAML 방식은 '간단하고 직관적'이어서 AI 에이전트를 '처음 만들 때' 주로 사용한다고
      명시하고 있습니다. 따라서 초기 개발에 부적합하다는 설명은 본문의 내용과 정반대입니다. 나머지 선택지 1, 2, 3번은 본문에서
      YAML 설정의 한계점으로 명확히 언급된 '재사용성 부족', '정적 구조', '타입 안정성 부재'에 각각 해당합니다.
  - question: 본문이 예측하는 미래의 '코드형 에이전트(Agent-as-Code)' 트렌드에 대한 설명으로 옳지 않은 것은 무엇입니까?
    ...
    answer: 3
    explanation: >-
      정답은 4번입니다. 본문에서 '코드형 에이전트'는 에이전트의 '정의'(프롬프트, 도구, 로직)를 코드로 관리하는 것을 의미하며, 이를
      통해 Git 기반 버전 관리, CI/CD, 테스트 자동화가 가능해진다고 설명합니다. 이는 1, 2, 3번 선택지와 일치합니다. 하지만
      AI 모델 자체의 학습 과정이나 파라미터를 직접 코드로 제어한다는 내용은 본문에서 언급되지 않았습니다. '코드형 에이전트'는 LLM을
      활용하는 애플리케이션의 구조와 동작을 코드로 관리하는 개념이지, LLM 모델 자체를 개발하는 MLOps와는 다릅니다.
---
