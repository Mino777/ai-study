...
applicable_to: ["any"]
connections: []
description: "AI 에이전트 스킬은 통과율(Pass Rate)이 거의 변하지 않아도 행동을 위험하게 바꿀 수 있다. 결과가 아닌 과정을 검증하는 '행동 감사(Behavioral Audits)' 방법론을 정리한다."
generated_by: gemini
quiz:
  - question: "행동 감사(Behavioral Audits)가 통과율(Pass Rate)보다 강조되는 이유는?"
    choices:
      - "통과율 계산이 너무 느려서"
      - "통과율이 거의 변하지 않아도 스킬이 행동을 위험하게 바꿀 수 있어서"
      - "통과율은 항상 100%로 고정되어서"
      - "행동 감사가 자동화하기 더 쉬워서"
    answer: 1
    explanation: "CTA 연구는 통과율 +0.3%p 변화 뒤에 522개의 행동 변화가 숨어 있었음을 보여준다."
  - question: "Behavioral Integrity Verification(BIV) 연구의 주요 발견은?"
    choices:
      - "모든 스킬이 선언대로 정확히 동작했다"
      - "분석된 스킬의 80% 이상에서 선언과 실제 행동이 불일치했다"
      - "스킬은 성능에 영향을 주지 않았다"
      - "스킬 설명은 불필요하다"
    answer: 1
    explanation: "BIV는 스킬 설명과 실제 행동의 불일치를 80% 이상에서 발견했고, 대부분은 악의보다 개발자 부주의에서 비롯됐다."
  - question: "SkillsBench 연구가 시사하는 올바른 결론은?"
    choices:
      - "스킬은 무조건 피해야 한다"
      - "자가 생성 스킬이 큐레이트된 스킬보다 항상 낫다"
      - "스킬은 신중히 검토·채택해야 하며, 큐레이트된 스킬이 효과적이다"
      - "스킬 채택 여부는 성능과 무관하다"
    answer: 2
    explanation: "큐레이트된 스킬은 평균 +16.2%p였지만 자가 생성 스킬은 평균 이득이 없었다. 결론은 '피하라'가 아니라 '신중히 채택하라'이다."
---

> 이 엔트리는 Blake Crosley의 [AI Agent Skills Need Behavioral Audits, Not Pass Rates](https://blakecrosley.com/blog/ai-agent-skills-need-behavioral-audits-not-pass-rates)를 정독하고 핵심을 추출한 것이다.
...
1.  [Counterfactual Trace Auditing of LLM Agent Skills](https://arxiv.org/abs/2605.11946) — 통과율이 거의 변하지 않아도 522개의 행동 변화(SIP) 패턴이 발견됨을 보고
2.  [Behavioral Integrity Verification for AI Agent Skills](https://arxiv.org/abs/2605.11770) — 49,943개 스킬 중 80%에서 선언과 실제 행동의 불일치를 발견
3.  [SkillsBench: Benchmarking How Well Agent Skills Work Across Diverse Tasks](https://arxiv.org/abs/2602.12670) — 큐레이트된 스킬은 평균 +16.2%p, 자가 생성 스킬은 평균 이득 0