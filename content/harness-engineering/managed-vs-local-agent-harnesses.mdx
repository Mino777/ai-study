...
applicable_to: ["any"]
connections: [harness-engineering/ai-agent-architecture-overview, harness-engineering/claude-code-native-features-migration-guide, agents/agent-architectures]
...
```mermaid
graph TD
    subgraph Local["Local Judgment Harness"]
        direction LR
        A["Product Taste"]
        B["Evidence Gates"]
        C["Project Memory"]
    ...
    subgraph Managed["Managed Agent Platform"]
        ...
        T["Tracing and Events"]
        E["Tool Execution Glue"]
    end

    Local -- "Provides Standards" --> Managed
    Managed -- "Executes and Reports" --> Local
...
원문이 "managed outcome"으로 부르는 패턴은 이 트렌드의 미래를 보여준다. 개발자가 평가 기준(rubric)을 정의하면, Managed 하네스가 별도의 평가자(grader)를 프로비저닝하여 에이전트가 그 피드백을 바탕으로 반복 작업을 수행한다. 이는 로컬의 '기준'을 런타임의 '슬롯'에 주입하는 패턴이다.
