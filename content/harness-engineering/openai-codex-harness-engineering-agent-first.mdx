...
description: >-
  OpenAI가 Codex 에이전트만으로 5개월간 프로덕션 소프트웨어를 구축한 실험 보고: Agent-to-Agent 리뷰, 환경 설계 원칙,
  구조적 문서화, Garbage Collection 패턴
...
quiz:
  - question: OpenAI가 '하나의 큰 AGENTS.md' 방식이 실패한 3가지 이유 중 하나가 아닌 것은?
    ...
    answer: 2
    explanation: >-
      모델은 파일을 읽을 수 있다. 문제는 컨텍스트 잠식, 우선순위 상실, 즉시 부패의 3가지. 이것이 CLAUDE.md를 200줄 이하로
      유지하고 Skills로 분리하는 이유와 같은 원리.
  - question: Codex 팀이 PR 리뷰를 Agent-to-Agent로 전환한 핵심 이유는?
    ...
    answer: 1
    explanation: >-
      에이전트가 사람보다 훨씬 빠르게 PR을 생성. 사람이 병목이 되면 전체 파이프라인이 멈춤. 수정은 저렴(에이전트가 바로 고침)하지만
      대기는 비쌈(사람 리뷰 기다리는 시간). 따라서 리뷰도 에이전트에게.
  - question: Codex의 'Garbage Collection' 패턴이란?
    ...
    answer: 1
    explanation: >-
      코드 드리프트(원칙에서 벗어남)를 '주기적 대청소'가 아닌 '지속적 소액 결제'로 해결. 대부분의 리팩토링 PR이 1분 내 자동
      머지. Fowler의 '하네스 드리프트' 안티패턴에 대한 실전 해법.
---
