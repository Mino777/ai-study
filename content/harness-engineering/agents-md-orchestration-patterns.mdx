...
3.  **모순된 우선순위(Contradictory Priorities)**: "빠르게 배포"와 "포괄적인 테스트 커버리지 확보"를 동시에 지시하면, 에이전트는 명시적 우선순위가 없어 검증 단계를 건너뛰고 코드 생성으로 직행한다. 모호하거나 명세가 부족한(underspecified) 작업을 다루는 `Ambig-SWE` 연구([arXiv:2502.13069](https://arxiv.org/abs/2502.13069))는, 에이전트가 질문(clarification)을 던지지 않고 비대화형으로 조용히 작업을 진행할 때 해결률이 크게 떨어진다는 점을 보고한다. 실제로 더 엄격한 벤치마크(SWE-Bench+ 등)에서 모호한 이슈를 정직하게 다루면 해결률이 절반 가까이 하락하는 경향이 일관되게 관찰된다.
...
이 엔트리는 Blake Crosley의 [AGENTS.md Patterns: What Actually Changes Agent Behavior](https://blakecrosley.com/blog/agents-md-patterns)를 정독하고 핵심을 추출한 것이다. 글에서 인용된 GitHub의 2,500개+ AGENTS.md 분석("대부분의 에이전트 파일은 기술적 한계가 아니라 너무 모호해서 실패한다") 및 모호한 작업을 다루는 `Ambig-SWE` 연구는 `AGENTS.md` 작성 시 모호함을 제거하고 기계가 해석 가능한 운영 절차를 제공하는 것이 얼마나 중요한지 뒷받침한다.
