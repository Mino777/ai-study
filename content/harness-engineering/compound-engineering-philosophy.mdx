...
connections:
  ...
  - agents/ai-agent-workflow-scaffolding-codify-senior-procedures
  ...
  - harness-engineering/agent-multiplexer-selection-guide
...
status: complete
description: >-
  ai-study 위키와 3 프로덕트(ai-study / moneyflow / tarosaju)의 공통 운영 철학. 한 스프린트가 다음
  스프린트를 위한 자산으로 자동 변환되는 사이클을 어떻게 설계했는지 박제. 12 원칙 + 12 referencing 엔트리에서 인용된 실제
  사례.
type: entry
quiz:
    ...
    choices:
      ...
      - >-
        *each unit of engineering work should make subsequent units easier—not
        harder* — 한 단위 작업이 다음 단위를 더 쉽게 만들도록 설계한다
      - 코드를 더 빠르게 작성하는 것
      - 테스트 커버리지 100%
    answer: 1
    explanation: >-
      Journal 000 베이스라인에서 직접 인용된 원문이다. 모든 운영 결정(hook / slash command / 박제 / 이식)이
      이 한 줄의 구현체다. 사이클 *N*의 산출물이 사이클 *N+1*의 입력이 되어야 복리가 작동한다.
  - question: '*사람의 기억력에 의존하는 가드*와 *행동에 박힌 가드*의 차이가 Compound Engineering에서 중요한 이유는?'
    choices:
      - 행동 가드가 더 빠르게 실행되어서
      - >-
        기억 의존 가드는 *100% 실패한다*. 사고 확률이 0이 아니라 *시간 함수*라서, 시간이 흐르면 반드시 발생. 복리가 쌓이는
        동안 같은 사고가 재발하면 축적이 무너진다
      - 행동 가드는 AI가 이해하기 쉬워서
      - 기억 가드는 설정이 복잡해서
    answer: 1
    explanation: >-
      Journal 003 squash merge trap과 wt-branch-structural-safety에서 반복되는 핵심. PR
      코멘트 / 안내 문서 / hook 메시지는 사람이 *읽고 기억하고 따라야* 작동한다. 이 체인 중 한 링크만 끊어져도 사고. 행동에
      박힌 가드(예: /wt-branch의 worktree 생성)는 체인 자체가 없어서 사고 구조가 없다.
  - question: 박제(CHANGELOG / 회고 / 솔루션)를 *매 사이클 끝*에 *빠짐없이* 수행하게 만드는 가장 강력한 방법은?
    choices:
      - 체크리스트를 매번 리뷰한다
      - >-
        *큰 워크플로를 한 슬래시 커맨드로 묶어* 부분 실행 자체를 불가능하게 만든다. 사용자가 /compound를 호출하면 4가지
        박제가 *순서대로 반드시* 실행됨
      - 팀원에게 상호 리마인드하게 한다
      - PR 템플릿에 체크박스를 넣는다
    answer: 1
    explanation: >-
      compound-automation-slash-commands 엔트리의 핵심 해석. *Compound 단계가 빠지면
      traditional engineering with AI assistance*가 된다. 빠짐을 막는 가장 강력한 방법이 *한 명령으로
      묶어서 부분 실행 자체를 불가능하게 만드는 것*. 이게 /compound가 작은 유틸이 아니라 큰 워크플로인 이유.
generated_by: gemini
---
...
**실제 사례**:
- Journal 005 — 다른 세션이 ai-study의 박제 패치를 *별도 안내 없이 발견해서 적용*. 전이성의 첫 라이브 검증.
- **양방향 전이**: Journal 014의 `/curate-inbound` — 워커 프로젝트(moneyflow/tarosaju)에서 발견한 패턴이 허브(ai-study)로 역방향 전이. 단방향(허브→워커)뿐 아니라 *워커→허브*도 전이 경로.
- **4레포 동시 전이**: aidy-architect의 Spec-Driven 패턴이 ai-study 위키에 박제 → 다시 moneyflow/tarosaju의 CI/CD 패턴(ADR-010 hybrid fallback)이 ai-study+aidy 3레포에 동시 이식. *M 프로젝트 × N 사이클* 복리의 실전.
...
**원칙**: 박제된 패턴이 다음 사이클에 *다시 발생하면 안 된다*. 재발 시 가드를 행동 레벨로 끌어올린다 (메모리 → 훅 → 워크플로).

**왜**: 재발은 *기존 가드가 기억 레벨*이라는 증거. 원칙 2로 되돌아가서 더 낮은 레이어에 박는다.
...
- **FP율 모니터링** — 승격된 validator의 FP가 높으면(90%+) 사용자가 경고를 무시하기 시작 → 진짜 문제도 놓침. 코드블록 밀집 파일 대상 stress test로 FP율 측정 후 판단 (validate-mdx Trap 3 stress test 사례: FP 90%+ 확인 → "수동 검토 플래그"로 역할 한정, 자동 게이트 승격 보류)
...
flowchart LR
  ...
  ingest["/ingest · 외부 입력 가공"] -. "주입" .-> P
  wt["/wt-branch · 안전한 시작점"] -. "감싸기" .-> W
  test["pre-commit hook · test-gate"] -. "강제" .-> R
  compound["/compound · 박제 자동화"] -. "박제" .-> C
  autoceo["/autoceo · N 라운드 자동"] -. "전체" .-> P
```
...
| [harness-journal-024-solution-to-validator-promotion](/wiki/harness-engineering/harness-journal-024-solution-to-validator-promotion) | 원칙 9 | Mermaid 따옴표 누락 4번째 재발 → `validate-content.mjs` warning 승격. *"N=3이면 validator 승격"* 원칙의 최초 라이브 적용 |
| [cross-project-pattern-porting-methodology](/wiki/harness-engineering/cross-project-pattern-porting-methodology) | 원칙 5 | 전이성을 *방법론*으로 일반화. 3 프로젝트 간 패턴 이식의 5단계 프로세스 |
| [aidy-journal-009-stall-detection-3way-dispatch-feature-sprint](/wiki/harness-engineering/aidy-journal-009-stall-detection-3way-dispatch-feature-sprint) | 원칙 3, 9 | Stall Detection 4단계 프로토콜 — *사고(워커 10분 정체)가 다음 사이클의 모니터링 프로토콜로 박제* |
...
ai-study + 3 프로덕트 현재 상태: **Stage 3~4**. 슬래시 커맨드 7개(`/compound`, `/autoceo`, `/wt-branch`, `/ingest`, `/cross-session-review`, `/curate-inbound`, `/projects-sync`)로 박제 누락률을 거의 0으로 줄였고, `/autoceo`로 N 라운드 자동 실행까지 운영 중. aidy-architect는 10라운드 자동 스프린트(v0.9→v1.0, +101 tests)를 Stage 4 수준에서 완주.
...
**Trigger**: 새로운 박제 패턴을 만들거나, 한 사이클의 산출물을 다음 사이클 입력으로 설계할 때. 프로젝트의 운영 철학 자체를 이해해야 할 때.

**Prerequisites**:
- [Harness Engineering Overview](/wiki/harness-engineering/harness-engineering-overview) — AI 하네스 기초
- [Harness Journal 000 - 베이스라인](/wiki/harness-engineering/harness-journal-000-baseline) — 실전 사이클 구조

### Actionable Steps
1. 12가지 원칙 이해하기 - 특히 원칙 2(행동 가드), 3(사고 입력), 8(부분 실행 불가능)
2. 한 사이클의 *산출물 3가지* 정의: CHANGELOG + 회고 + 솔루션
3. 그 산출물들이 *다음 사이클의 정확한 입력*이 되도록 구조화
4. 박제 자체를 *한 슬래시 커맨드*로 묶기 (부분 실행 차단)
5. "다음에 적용할 것" 섹션이 *실제로 다음 큐*가 되는지 확인
6. 사고 발생 시 그걸 *실패가 아니라 입력*으로 박제
7. 한 사이클에 *한 박제만* - 1 → 3 → 9 점진적 적용
8. 체크리스트 12개 항목 (자기 점검 섹션) 확인 - Stage 몇 수준인지 자가 진단

### Anti-patterns
- ❌ 한 사이클에 13개 박제를 동시에 추진
- ❌ 박제 문서를 일반 블로그처럼 작성 (사람 대상)
- ❌ 사고를 실패로 취급하고 묻기
- ❌ 기억에만 의존하는 가드 (행동 레벨로 끌어올리지 않음)
- ❌ 박제 없이 코드만 머지
...
### 2026-04-18 — 깨진 문자 복원 + 경험 기반 보강

- **깨진 문자 복원**: 원칙 9의 한글 텍스트 2줄 복원 (Unicode replacement character `�` → 원문 복원)
- **원칙 5 전이성 보강**: 양방향 전이(워커→허브 `/curate-inbound`) + 4레포 동시 전이(ADR-010) 사례 추가
- **원칙 9 FP 모니터링 추가**: validate-mdx Trap 3 stress test 교훈 — 승격된 validator의 FP율이 높으면 경고 무시 유발 → stress test로 역할 한정 판단
- **5단계 Ladder 현재 상태 갱신**: 슬래시 커맨드 7개 + aidy-architect Stage 4 완주 반영
- **referencing 테이블 3건 추가**: Journal 024(N=3 승격), cross-project-porting(전이성 방법론), Aidy Journal 009(Stall Detection)
