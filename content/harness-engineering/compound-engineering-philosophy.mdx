...
요지는 단순하다. "복리(compound)"는 비유가 아니라 **설계 목```mermaid
flowchart LR
...
**Trigger**: 새 프로젝트 시작 시 / 스프린트 방법론 결정 시 / "다음 작업이 더 쉬워지는 구조"를 설계해야 할 때
...
**Actionable Steps (4단계 루프 적용)**:
1. **Plan** — 작업 전에 `docs/plans/<feature>.md`에 데이터 모델 + 변경할 파일 + 아키텍처 결정 기록. *AI가 질문 없이 실행 가능한 수준*까지 구체화.
2. **Work** — 한 사이클 = 한 가지 작업. PR 단위로 작게 쪼개기. `/wt-branch`로 worktree 분기해서 시작.
3. **Review** — 발견사항을 P1(꼭 고침) / P2(고치는 게 좋음) / P3(있으면 좋음)으로 분류. 자동 게이트(test-gate.yml)가 primary reviewer.
4. **Compound** — `/compound` 슬래시 커맨드로 자동. CHANGELOG + retro + solutions + 메모리 동기화 한 번에. *Compound 단계를 생략하면 traditional engineering with AI assistance*.

**5단계 Ladder 체크**:
- 자기 위치가 Stage 3(plan-first, PR-only review) 이상인지 확인
- Stage 2(line-by-line review)에 머물고 있으면 *처리량이 사람 1명 = 사람 1명*으로 고정. `/autoceo`가 Stage 4로 올라가는 다리.

**Anti-patterns**:
- ❌ Compound 단계 생략 (원문: *"Skip it, and you've done traditional engineering with AI assistance"*)
- ❌ Plan이 너무 가벼움 ("할 일 한 줄 → 바로 구현")
- ❌ Line-by-line review에 묶여 있음 (Stage 2 plateau)
- ❌ *"빌드 통과 = 끝"* 착각 — 같은 버그를 3주 뒤에 또 만남
- ❌ 첫 시도 95% garbage rate를 못 견디고 루프 멈춤
