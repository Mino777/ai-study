...
**왜**: 재발은 *기존 가드가 기억 레벨*이라는 증거. 원칙 2로 되돌아가서 더 낮은 레이어로 박는다.

**실제 사례**: Journal 003 squash merge trap — 사고가 *두 번* 발생. 두 번째 발생 후 문서 가드(메모리 레벨)로는 부족하다는 결론. 결국 `/wt-branch`(행동 레벨)로 박음.
...
**실제 사례**: compound-automation-slash-commands의 4개 슬래시 커맨드 매핑 — `/ingest`(Plan 입력), `/wt-branch`(Work 시작), `/compound`(Compound 단계), `/autoceo`(전체 루프 × N 라운드). 4개 합치면 4단계가 다 커버됨.
...
## 12 referencing 엔트리 — 어디서 어떻게 인용되는가
...
- [ ] 같은 사고가 *재발*한 적이 있는가 (원칙 9 — 있다면 가드 레벨 상향 필요)
