...
description: >-
  이 글은 AI 에이전트 시스템의 전반적인 아키텍처를 다루며, 에이전트의 설계 및 오케스트레이션에 대한 심층적인 이해를 돕습니다. 하네스
  엔지니어링 관점에서 에이전트 구성 요소와 상호 작용 방식을 제시합니다. 자율 에이전트 시스템 구축을 위한 기본 청사진을 제공합니다.
generated_by: gemini
quiz:
...
> 이 엔트리는 Blake Crosley가 정리한 "Agent Architecture: Building AI-Powered Development Harnesses"([blakecrosley.com](https://blakecrosley.com))를 정독하고 핵심을 추출한 것이다.
...
```mermaid
graph TD
    subgraph harness ["하네스 아키텍처 Harness Architecture"]
        direction LR
        A["Hooks (강제 실행)"] -- "보장성" --> B
...
---
이 엔트리는 Blake Crosley가 정리한 "Agent Architecture: Building AI-Powered Development Harnesses"([blakecrosley.com](https://blakecrosley.com))를 정독하고 핵심을 추출한 것입니다.
