...
status: complete
description: >-
  Garry Tan의 Skillify 글을 /ingest로 정리한 세션 당일, Step 8(check-resolvable)을 허브에 구현하고
  자기 프로젝트 9개(허브 1 + 워커 6 + 추가 iOS 2)에 dogfood. 워커 6개가 100% dark 상태였다는 발견과 28개
  dark skill을 0으로 만든 3시간 기록. 숫자 위주, 서사 최소.
type: entry
quiz:
  - question: 이번 실측에서 발견된 워커 6개의 초기 Skill routing 상태는?
    ...
    answer: 2
    explanation: >-
      mino-moneyflow/tarosaju는 routing 12건 있으나 자기 로컬
      3개(autoceo/compound/wt-branch) 미매핑, aidy-architect는 로컬 10개 중 9개 dark,
      aidy-server/ios/android는 routing 섹션 자체 없음. Garry Tan의 '15% dark'보다 훨씬 심각.
  - question: 8 repo 동시 롤아웃에서 Journal 003 squash-merge 트랩이 재현된 방식은?
    choices:
      - 수동 rebase 중 충돌
      - >-
        ai-review.yml이 PR을 squash 머지 → 로컬 skillify-phase-a가 orphan SHA가 되어
        fast-forward 머지 불가
      - 워커가 허브 브랜치를 force-push
      - merge-base 계산 오류
    answer: 1
    explanation: >-
      Journal 003에서 박제한 트랩이 8배로 증폭 재발. 원격은 squash 결과로 정상 머지됐지만 로컬 브랜치는
      'pre-squash SHA'로 떠서 ff 불가. 해결은 ff 시도 대신 브랜치/worktree 정리.
  - question: 측정 후 '스프린트 한복판에 발견'된 pre-existing 이슈는?
    choices:
      - ai-study main이 origin과 완전 분기
      - >-
        aidy-architect 로컬 main에 2026-04-22 unpushed 커밋(29a7669,
        dispatch+gate-log+AGENT_TEAMS) 존재 — 세션 시작 시 projects-sync를 안 해서 뒤늦게 감지
      - 워커 프로젝트 전원 uncommitted 변경 보유
      - 추가 iOS repo의 Bitbucket 브랜치 삭제됨
    answer: 1
    explanation: >-
      Anti-rationalization 관점: '다른 세션의 작업' 탓이 아니라 '사전 점검 스킵'이 내 실수.
      feedback_projects_sync_includes_aidy.md 룰이 존재하는데 반사적으로 트리거 못 함.
      cherry-pick PR #15로 복구 완료.
---
...
| iOS (추가 1) | 15 | ❌ 섹션 없음 | 14 (1개는 DEPRECATED) | — |
...
| iOS (추가 1) | 섹션 신규 + orchestrate-layer DEPRECATED 삭제 | 0/0 ✅ 로컬 (Bitbucket auth 대기) |
