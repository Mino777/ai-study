---
title: >-
  Addy Osmani — Agent Harness Engineering 정리: "You're not the model, you're the
  harness"
...
status: complete
description: >-
  Addy Osmani가 2026년 4월에 정리한 'Agent Harness Engineering' 포스트를 한국어로 옮기고 내 위키 관점에서
  재해석. "Agent = Model + Harness" 등식, 실패가 곧 규칙이 되는 래칫(ratchet), 모델-하네스 공진화 루프,
  Harness-as-a-Service 흐름을 정리.
type: entry
quiz:
  - question: Addy Osmani가 정의한 'Agent = Model + Harness'에서 '하네스'에 포함되지 않는 것은?
    ...
    answer: 2
    explanation: >-
      하네스는 모델을 '둘러싸는' 코드·설정·실행 로직이다. 모델 자체(가중치/학습 데이터)는 하네스가 아니라 그 안에 끼워 넣는
      부품이다. 그래서 Osmani는 "you're not the model, you're the harness"라고 못 박는다 — 우리가
      손댈 수 있는 표면은 하네스뿐이다.
  ...
  - question: '포스트가 강조하는 후크(hook)의 출력 원칙 "success is silent, failures are verbose"의 의도는?'
    ...
    answer: 1
    explanation: >-
      성공 시 후크가 떠드는 만큼 에이전트의 컨텍스트 윈도우가 낭비되고, 실패 시 정보가 부족하면 자기 교정 루프가 끊긴다. 그래서 성공
      = 침묵, 실패 = 풍부한 컨텍스트라는 비대칭이 권장된다 — 본 위키의 `bash-watcher-zero-token-polling`
      패턴과 같은 결의 사고다.
---
