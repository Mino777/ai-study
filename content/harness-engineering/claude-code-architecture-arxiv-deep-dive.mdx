...
description: >-
  arxiv 논문 'Dive into Claude Code'와 2026.03 소스 유출 분석으로 밝혀진 Claude Code 내부 아키텍처:
  5단계 컨텍스트 압축, 27개 Hook 이벤트, auto mode classifier, 서브에이전트 격리 패턴
...
quiz:
  - question: Claude Code의 5단계 컨텍스트 압축에서 가장 마지막에 실행되는 단계는?
    ...
    answer: 2
    explanation: >-
      5단계: Budget Reduction → Snip → Microcompact → Context Collapse →
      Auto-compact. 앞 단계가 충분하면 뒤 단계는 실행 안 됨. Auto-compact는 가장 비싸고 마지막 수단.
  - question: Auto mode에서 Sonnet 4.6 classifier가 모델의 prose(자연어 설명)를 보지 않는 이유는?
    ...
    answer: 1
    explanation: >-
      모델이 'This is a safe operation because...'식으로 classifier를 sweet-talk하는 것을
      원천 차단. classifier는 사용자 요청 + 도구 호출만 보고 판단.
