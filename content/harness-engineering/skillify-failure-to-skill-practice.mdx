...
status: complete
description: >-
  Garry Tan의 'skillify' 관례 — 모든 에이전트 실패를 10단계 체크리스트(SKILL.md → 결정론적 코드 →
  단위/통합/LLM 평가 → 리졸버 + 라우팅 평가 → DRY 감사 → E2E 스모크 → 브레인 파일링)로 전환해 재발을 구조적으로 불가능하게
  만든다. LangChain이 주는 '조각(pieces)'과 박혀있는 '관례(practice)'의 차이.
type: entry
...
quiz:
  - question: Garry Tan이 구분하는 'latent 작업'과 'deterministic 작업'의 핵심 차이는?
    choices:
      ...
      - >-
        latent은 판단이 필요하고, deterministic은 같은 입력에 같은 출력이 나오는 정밀 작업. 캘린더 grep 같은 건
        deterministic인데 에이전트가 latent space에서 처리하면 버그
      - latent은 캐시되고 deterministic은 캐시 안 됨
      - 'latent은 프로덕션용, deterministic은 테스트용'
    answer: 1
    explanation: >-
      Garry Tan이 말하는 버그는 '틀린 답'이 아니라 '틀린 쪽(wrong side)'에서 처리한 것. 3줄 스크립트로 끝날
      결정론적 작업을 에이전트가 latent에서 추론하면 느리고, 틀리고, 재현 안 된다.
  - question: Skillify 10단계 중 Hermes Agent의 skill_manage 도구가 '누락하는 부분'에 해당하는 것은?
    ...
    answer: 2
    explanation: >-
      기사가 지적하는 Hermes의 갭은 '생성은 되나 검증이 없다' — 같은 도메인에 유사 skill 2개가 생겨 ambiguous
      routing이 되어도 못 잡는다. GBrain의 `gbrain doctor`가 check-resolvable + DRY audit을
      맡는 이유.
  - question: Skillify의 '재발 구조적 불가능'이 의미하는 바는?
    ...
    answer: 2
    explanation: >-
      소프트웨어 엔지니어링이 2005년에 푼 문제 — '버그마다 테스트, 테스트는 영원히 산다'의 에이전트 버전. 프롬프트 튜닝은
      컨텍스트가 복잡해지는 순간 decay하지만, 구조적 gate는 안 decay한다.
---
