...
date: "2026-04-12"
tags: [harness-journal, harness-engineering, ai-ops, baseline, audit, compound-engineering]
confidence: 4
connections:
...
status: complete
description: "Harness Journal 시리즈의 출발점. mino-moneyflow와 mino-tarosaju 두 프로젝트의 AI 운영 인프라를 병렬 감사로 전수 조사하고, 공통 갭 8개와 첫 5 사이클 큐를 정의."
type: entry
quiz:
  - question: "Harness Journal 시리즈의 0번 엔트리(베이스라인)가 왜 *코드 변경 전*에 작성되어야 하는가?"
    ...
    answer: 1
    explanation: "베이스라인은 \"무엇이 있고 무엇이 없는지\"를 *추측 없이* 박제하는 작업이다. 추측으로 시작하면 첫 갭 식별부터 가짜가 되고, 효과 측정의 비교 기준점이 사라진다. 시리즈 전체가 베이스라인에 의존한다."
  - question: "두 프로젝트의 AI 운영 인프라 감사에서 가장 자주 등장한 *공통* 갭은?"
    ...
    answer: 1
    explanation: "양쪽 모두 입력 검증(Zod, parseJsonBody)은 잘 갖췄지만, *LLM 응답 자체*를 schema.parse로 거르지 않는다. moneyflow의 13 에이전트 출력, tarosaju의 fortune/tarot-chat 모두 동일. 환각이 그대로 사용자에게 반환될 수 있는 구조."
  - question: "두 프로젝트가 \"비대칭\"이라는 게 왜 *복리 효과의 출발점*인가?"
    choices:
      - "프로젝트가 다르면 코드도 다르므로 호환이 안 돼서 별도 작업이 필요"
      - "한쪽에 이미 검증된 자산(예: moneyflow의 Circuit Breaker, tarosaju의 RLS 정적 감사)이 다른 쪽에는 없다는 뜻이고, 그 이식 작업이 곧바로 다음 사이클의 큐가 되기 때문"
      - "비대칭 자체가 위험 신호이므로 즉시 통일해야 함"
      - "비대칭이 있는 쪽이 항상 품질이 낮으므로 개선 우선순위가 명확해짐"
    answer: 1
    explanation: "한쪽이 이미 만들어둔 패턴은 *증명된 자산*이다. 다른 쪽으로 이식하는 작업은 처음부터 만드는 것보다 훨씬 싸다. Compound Engineering이 말하는 \"다음 작업이 더 쉬워진다\"가 *프로젝트 간*에도 작동한다는 첫 사례."
---
...
| 8 | **`.claude/skills/` 미활용** | 양쪽 모두 슬래시 커맨드 2개(`/compound`, `/autoceo`)만. *작은 재사용 단위*인 skill 형식 미사용. | 낮음~중간 |
