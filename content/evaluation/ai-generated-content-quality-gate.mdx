...
description: >-
  AI가 자동 생성한 위키 엔트리의 품질을 사전 차단하는 6가지 게이트. Gemini 파이프라인에서 실측 없는 이론, SEO 스팸,
  Mermaid 에러를 구조적으로 방지한다.
quiz:
  - question: AI 자동 생성 콘텐츠에서 가장 위험한 품질 문제는?
    ...
    answer: 1
    explanation: >-
      '98% 절감', '100만 스케일' 같은 검증 안 된 수치를 AI가 자신 있게 생성하면, 독자가 사실로 받아들일 위험이 크다.
      문법이나 분량은 쉽게 교정 가능하지만, 근거 없는 주장은 위키 신뢰도 자체를 훼손한다.
  - question: Mermaid 다이어그램에서 AI가 가장 자주 만드는 에러는?
    ...
    answer: 1
    explanation: >-
      AI는 자연스럽게 A[Label (with parens)] 형태를 생성하지만, Mermaid 파서는 괄호를 서브그래프 문법으로
      해석한다. A["Label (with parens)"]로 따옴표 감싸기가 필수. 백틱과 콜론도 동일한 문제를 일으킨다.
  - question: AI 생성 콘텐츠의 slug 규칙으로 올바른 것은?
    ...
    answer: 2
    explanation: >-
      긴 slug는 SEO 스팸으로 의심받고, 한글 slug는 404를 유발한다. 40자 이내 영문 kebab-case가 표준.
      'sandbox-diff-based-ai-agent-token-reduction' 같은 키워드 나열은 안티패턴.
---
