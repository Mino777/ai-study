...
status: complete
description: >-
  Flow Map 시리즈 7편. iOS 개발자가 프로덕션에서 체화한 'XCTest + CI 통과 = 안전' 관성을 벗고, aidy
...
```mermaid
flowchart TD
    WO["WO-NNN: DELETE /api/memories/id · 서버/iOS/Android 3 구현 + 테스트"]

    subgraph Preload ["워커 세션 시작 로딩"]
      PL1["api-contract.md § 3. Memory · + Error Codes (MEMORY_NOT_FOUND · FORBIDDEN)"]
    ...
    subgraph Server["서버 워커 Kotlin"]
      S_Unit["MemoryServiceTest · deleteMemory_rollsBack_whenApiFails"]
      S_Ctrl["MemoryControllerTest @WebMvcTest · 204 · 404 MEMORY_NOT_FOUND · 403 FORBIDDEN · 401"]
      S_Run["./gradlew test · → BUILD SUCCESSFUL + NN passed"]
      S_Evidence["커밋 메시지: · 테스트: 42 passed / 0 failed"]
    end
    ...
    subgraph iOS["iOS 워커 Swift/TCA"]
      I_Feat["MemoryFeatureTests · delete_happyPath · delete_notFound · delete_forbidden"]
      I_Api["APIClientTest (MockURLProtocol)"]
      I_Run["xcodebuild test -workspace · → TEST SUCCEEDED · NN tests executed"]
      I_Check{"'no tests to run' · 또는 'Executed 0 tests' · 출력됐는가?"}
      I_Fail["테스트 실제로 안 돈 버그! · (Session 4 사고 패턴)"]
      I_Evidence["커밋 메시지: · 테스트: 87 passed / 0 failed"]
    end
    ...
    subgraph Android["Android 워커 Compose"]
      ...
      A_Run["./gradlew testDebugUnitTest · → NN tests, 0 failures"]
      A_Evidence["커밋 메시지: · 테스트: 56 passed / 0 failed"]
    end
    ...
    Merge --> Compound["/compound · retro · solution · ADR 박제"]
...
| **iOS Feature 테스트** | TestStore 로 Reducer state/action 시퀀스 검증 | 프로덕션에서 익숙한 패턴 (TCA 경험자) 또는 MVVM ViewModel 단위 테스트 |
| **iOS APIClient 테스트** | URLSession 실호출 금지 → MockURLProtocol | 프로덕션 앱의 네트워크 모킹 패턴 |
| **'no tests to run' 검출** | Test Suite 실제 실행됐는가 line-level 확인 | 흔히 놓치는 함정 — 프로덕션 CI 에서도 같은 위험 있음 |
| **Android ViewModel · Repository 테스트** | Compose State 검증 + MockWebServer | 엔터프라이즈 Android 팀과 구조 동일 |
...
| **TestStore (TCA)** | RIBs/ReactorKit 기반 프로덕션의 유사 테스트 | Action send → state 전이 + receive → effect 검증 |
| **`@Dependency` + `testValue = unimplemented()`** | RIBs `Dependency` 프로토콜 Mock 주입 | *호출 안 한 의존성* 이 실수로 호출되면 테스트 즉시 실패 (엄격함 차원이 다름) |
| **MockURLProtocol** | URLSession stub | 프로덕션 패턴과 동일 |
| **@MockkBean** | SpringMock — Anthropic API 격리 | 필수 |
| **H2 테스트 DB** | 프로덕션 SQLite 또는 Docker test containers | 가장 빠른 옵션 |
...
| **Tuist PackageSettings** | 프로덕션 SPM 직접 관리 경험 | .framework 명시 누락 → 테스트 조용한 실패 |
| **@Disabled 금지** | XCTSkip · @Ignore 오용 방지 | 예외는 반드시 주석 + TODO |
| **한국어 에러 메시지 assert** | 프로덕션 i18n 테스트 | Bean Validation 메시지 검증까지 |
| **통합 테스트 (E2E 시나리오)** | 프로덕션 staging 테스트 | signup → login → 기능 사용 1 시나리오 필수 |
