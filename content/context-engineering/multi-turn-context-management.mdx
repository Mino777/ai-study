...
date: "2026-04-12"
tags: [multi, turn, context, management, context-engineering]
confidence: 1
connections: [context-engineering/context-compression, context-engineering/context-engineering-fundamentals]
status: draft
...
quiz:
  - question: 본문에 따르면, 대화형 AI 시스템에서 '멀티턴 컨텍스트 관리'가 필수적인 가장 근본적인 이유는 무엇인가요?
    ...
    answer: 1
    explanation: 본문은 "LLM 자체는 이전 대화 내용을 '기억'하지 못합니다. 우리가 흔히 대화형 AI 에이전트라고 부르는 시스템이 자연스러운 대화를 이어갈 수 있는 것은, 개발자가 명시적으로 이전 대화의 맥락(context)을 다음 요청에 함께 전달하기 때문입니다."라고 명시하며 LLM의 본질적인 stateless한 특성을 가장 근본적인 이유로 제시하고 있습니다. 다른 선택지들은 컨텍스트 관리의 목표나 한계점에 해당합니다.
  - question: 대화 기록 전체를 LLM에 전달하는 방식의 한계점으로 본문에서 언급되지 않은 것은 무엇인가요?
    ...
    answer: 3
    explanation: 본문에서 대화 기록 전체를 LLM에 전달하는 방식의 한계점으로 토큰 제한, 비용 증가, 응답 속도 저하, 환각 위험 증가를 언급하고 있습니다. LLM이 이전에 생성한 잘못된 정보를 자동으로 수정하는 기능이 상실된다는 내용은 한계점으로 언급되지 않았습니다. 오히려 너무 많은 정보를 제공하면 환각 위험이 증가할 수 있다고 명시되어 있습니다.
  - question: "'슬라이딩 윈도우' 컨텍스트 관리 패턴에 대한 설명으로 옳은 것은 무엇인가요?"
    ...
    answer: 2
    explanation: 슬라이딩 윈도우 패턴은 최신 N개의 메시지나 특정 토큰 수 이내의 메시지만 유지하여 LLM에 전달하고 오래된 메시지는 삭제합니다. 이는 컨텍스트 길이를 예측 가능하게 유지하여 비용과 속도를 제어하기 용이하다는 장점이 있습니다. 선택지 1은 슬라이딩 윈도우의 단점인 정보 손실을 부정하므로 틀렸습니다. 선택지 2는 최신 메시지 대신 오래된 메시지를 우선시한다고 하므로 틀렸습니다. 선택지 4는 구현이 복잡하다고 하여 본문의 '구현이 매우 간단합니다'와 상반되며, 최신 대화 집중이라는 장점과도 배치됩니다.
