...
```mermaid
flowchart TD
    Trigger["iOS 워커가 구현 중 발견: · 'Memory 목록 Response에 totalCount 있으면 · 페이지네이션 UI 구현이 쉬움'"]

    subgraph WorkerSide["워커 세션 pane 2/3"]
      W_stop["작업 일시 중단"]
      W_req["inbox/ios-request.md 작성 · - 배경 / 요청 / 대안 / 블로킹 여부"]
    end
    ...
    subgraph Watcher ["Architect 알림"]
      Watch["inbox-watcher.sh 또는 /monitor · → macOS notification"]
    end
    ...
    Decide1 -->|inbox/ios-response.md| W_resume1["워커 재개 · (기존 계약 기준 우회 구현)"]

    subgraph ContractUpdate ["계약 업데이트"]
      EditSpec["api-contract.md 수정 · § 3. Memory 스키마에 totalCount 추가"]
      VersionBump["버전 히스토리 append · v0.2.1 → v0.2.2 '+totalCount'"]
      ADR["필요 시 specs/decisions/ · ADR-XXX 작성"]
    end
    ...
    subgraph GateLoop ["Gate 1 대조 루프"]
      G1_server["/gate-1 server · → totalCount 필드 존재 확인"]
      G1_ios["/gate-1 ios · → 디코더에 totalCount 있는지"]
      G1_android["/gate-1 android · → data class에 totalCount 있는지"]
    end
    ...
    Merge --> Compound["/compound · → ADR 공개 (결정 근거 박제)"]
```
