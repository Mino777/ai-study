...
quiz:
  ...
  - question: 다음 중 기술 블로그 본문에서 '동적 맥락의 주요 유형'으로 언급되지 않은 것은 무엇입니까?
    ...
    answer: 1
    explanation: 본문 '동적 맥락의 주요 유형' 섹션에는 '사용자 세션 맥락', '시스템/환경 맥락', '외부 실시간 데이터 맥락'의 세 가지가 제시되어 있습니다. 선택지 중 '사용자의 현재 장바구니에 담긴 상품 목록'은 '사용자 세션 맥락'에 해당하며, '현재 접속한 디바이스의 운영체제 버전'은 '시스템/환경 맥락'에, '외부 API 호출을 통해 실시간으로 가져온 날씨 정보'는 '외부 실시간 데이터 맥락'에 해당합니다. 'AI 에이전트의 내부 모델 학습 데이터 셋'은 AI 모델 자체의 구성 요소이며, 실시간으로 변화하는 외부/사용자/환경 맥락 정보와는 관련이 없으므로, 동적 맥락의 주요 유형으로 언급되지 않았습니다.

---

## 왜 동적 맥락 주입이 필수적인가?

오늘날 AI 시스템, 특히 LLM(Large Language Model) 기반의 에이전트는 점점 더 복잡하고 동적인 환경에서 사용자와 상호작용합니다. iOS 앱의 개인 비서부터 웹 서비스의 이커머스 어시스턴트에 이르기까지, AI가 효과적으로 기능하려면 실시간으로 변화하는 수많은 정보를 이해하고 활용할 수 있어야 합니다.
