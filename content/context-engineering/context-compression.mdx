...
quiz:
  - question: "Context Compression이 필요한 주요 이유 4가지 중, RAG 시스템에서 가장 직접적인 효과는?"
    choices:
      ...
      - "코드 복잡도 감소"
    answer: 1
    explanation: "RAG에서 retriever가 Top-K 문서를 많이 가져올수록 context가 커진다. Context Compression은 이 검색 결과를 LLM 투입 전에 핵심만 추출해 최종 LLM으로 가는 토큰을 50~75% 감소시킨다."
  - question: "Post-Retrieval Compression 패턴에서 Compressor LLM을 별도로 사용하는 이유는?"
    choices:
      ...
      - "모델 간 성능 비교를 위해"
    answer: 1
    explanation: "Compressor가 '이 질문과 가장 관련된 부분만 추출'하는 지능형 필터 역할을 한다. 결과적으로 최종 LLM은 수집된 전체 문서가 아닌 정제된 핵심 정보만 받아 토큰도 줄고 정확도도 높일 수 있다."
  - question: "iOS/프론트엔드 개발에서 Context Compression이 특히 중요한 이유는?"
    choices:
      ...
      - "클라이언트 사이드 캐싱 때문"
    answer: 0
    explanation: "모바일은 서버 환경과 달리 네트워크 대역폭 제한, 배터리 제약, 사용자가 느끼는 응답 지연이 모두 중요하다. Context Compression으로 네트워크 페이로드를 줄이고 처리 속도를 높이고 배터리 소모를 최소화할 수 있다."
connections:
