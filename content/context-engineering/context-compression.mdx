...
applicable_to:
  - any
quiz:
  - question: 'Context Compression이 필요한 주요 이유 4가지 중, RAG 시스템에서 가장 직접적인 효과는?'
    choices:
      - 네트워크 대역폭 절감
      - 검색된 모든 문서를 LLM이 처리해야 하는 토큰 수를 획기적으로 감소
      - 데이터베이스 저장 공간 절감
      - 코드 복잡도 감소
    answer: 1
    explanation: >-
      RAG에서 retriever가 Top-K 문서를 많이 가져올수록 context가 커진다. Context Compression은 이
      검색 결과를 LLM 투입 전에 핵심만 추출해 최종 LLM으로 가는 토큰을 50~75% 감소시킨다.
  - question: Post-Retrieval Compression 패턴에서 Compressor LLM을 별도로 사용하는 이유는?
    choices:
      - 비용을 더 많이 써서 정확도를 높이기 위해
      - 검색된 여러 문서를 하나의 LLM이 압축하면 노이즈를 제거하고 최종 LLM의 컨텍스트를 최적화할 수 있기 때문
      - 병렬 처리를 위해
      - 모델 간 성능 비교를 위해
    answer: 1
    explanation: >-
      Compressor가 '이 질문과 가장 관련된 부분만 추출'하는 지능형 필터 역할을 한다. 결과적으로 최종 LLM은 수집된 전체
      문서가 아닌 정제된 핵심 정보만 받아 토큰도 줄고 정확도도 높일 수 있다.
  - question: iOS/프론트엔드 개발에서 Context Compression이 특히 중요한 이유는?
    choices:
      - '모바일 환경의 네트워크 제약, 배터리 소모, 응답 지연을 함께 해결할 수 있기 때문'
      - iOS 앱의 UI가 복잡하기 때문
      - 프론트엔드 성능이 최우선이기 때문
      - 클라이언트 사이드 캐싱 때문
    answer: 0
    explanation: >-
      모바일은 서버 환경과 달리 네트워크 대역폭 제한, 배터리 제약, 사용자가 느끼는 응답 지연이 모두 중요하다. Context
      Compression으로 네트워크 페이로드를 줄이고 처리 속도를 높이고 배터리 소모를 최소화할 수 있다.
connections:
...
## TypeScript 구현 예제: Context Compression RAG

TypeScript/JavaScript 환경에서는 LangChain.js를 사용하여 유사한 Context Compression RAG를 구현할 수 있습니다. 다음은 실제 프로덕션 환경에 가까운 TypeScript 예제입니다.
...
class ContextCompressionRAG {
  ...
  /**
   * 문서 모음을 기반으로 벡터 스토어 구성
   */
  async initializeVectorStore(documents: string[]): Promise<void> {
   ...
   * 쿼리 기반으로 문서 검색 (압축 없음)
   */
  async retrieveWithoutCompression(
    query: string,
    k: number = 5
  ): Promise<Document[]> {
    if (!this.vectorStore) {
  ...
  /**
   * Context Compressor를 통한 문서 압축
   */
  async compressRetrievedDocuments(
    query: string,
    retrievedDocs: Document[]
  ): Promise<string> {
    const compressorPrompt = PromptTemplate.fromTemplate(`
다음은 검색된 문서들입니다. 주어진 질문과 가장 관련성이 높은 부분만 추출하여 간결하게 요약해 주세요.
...
`);
    ...
    const documentTexts = retrievedDocs
  ...
  /**
   * 최종 답변 생성 (압축된 컨텍스트 사용)
   */
  async generateAnswerWithCompression(
    query: string,
    compressedContext: string
  ): Promise<string> {
    const answerPrompt = PromptTemplate.fromTemplate(`
다음 정보를 기반으로 질문에 답변해 주세요.
...
`);
   ...
   * 전체 RAG 파이프라인 (압축 포함)
   */
  async executeRAGWithCompression(
    query: string,
    k: number = 5
  ): Promise<{
    answer: string;
    ...
    // 1. 검색
    const retrievedDocs = await this.retrieveWithoutCompression(query, k);
    const originalLength = retrievedDocs.reduce(
      (sum, doc) => sum + doc.pageContent.length,
      0
    );

    // 2. 압축
    const compressedText = await this.compressRetrievedDocuments(
      query,
      retrievedDocs
    );
    ...
    // 3. 답변 생성
    const answer = await this.generateAnswerWithCompression(
      query,
      compressedText
    );
   ...
   * 비교: 압축 없는 RAG
   */
  async executeRAGWithoutCompression(
    query: string,
    k: number = 5
  ): Promise<string> {
    const retrievedDocs = await this.retrieveWithoutCompression(query, k);
    ...
    return this.generateAnswerWithCompression(query, documentTexts);
  }
}
...
async function demonstrateContextCompression() {
  const documents = [
    '프론트엔드 개발은 HTML, CSS, JavaScript를 사용하여 사용자 인터페이스를 구축하는 작업입니다. 사용자 경험(UX)과 반응 속도는 프론트엔드 개발의 핵심 고려사항입니다.',
    'React는 Facebook에서 개발한 JavaScript 라이브러리로, 컴포넌트 기반의 UI 구축을 가능하게 합니다. Virtual DOM을 통해 성능 최적화를 달성합니다.',
    'TypeScript는 JavaScript의 상위 집합으로, 정적 타입 체킹을 제공합니다. 대규모 프로젝트에서 코드의 안정성과 유지보수성을 향상시킵니다.',
    'Next.js는 React 기반의 풀스택 프레임워크로, 서버 사이드 렌더링(SSR)과 정적 생성(SSG)을 지원합니다. SEO 최적화와 성능 향상에 유리합니다.',
    'CSS-in-JS는 JavaScript 내에서 CSS를 작성하는 방식으로, Styled Components, Emotion 등의 라이브러리가 있습니다. 동적 스타일 관리가 가능합니다.',
    'AI 기반 프론트엔드 최적화는 사용자 행동 분석과 동적 콘텐츠 제공을 가능하게 합니다. LLM을 활용한 개인화 추천은 사용자 만족도를 높입니다.',
  ];

  const rag = new ContextCompressionRAG('gpt-4o-mini');
  await rag.initializeVectorStore(documents);

  const query = '프론트엔드 개발에서 성능 최적화는 어떻게 하나요?';

  console.log('=== Context Compression RAG 비교 ===\n');

  // 1. 압축 없는 RAG
  console.log('[1] 압축 없는 RAG');
  const answerWithoutCompression =
    await rag.executeRAGWithoutCompression(query);
  console.log('답변:', answerWithoutCompression);

  // 2. 압축 포함 RAG
  console.log('\n[2] Context Compression RAG');
  const result = await rag.executeRAGWithCompression(query);
  console.log('압축 통계:');
  console.log(`- 원본 크기: ${result.compression.originalLength} 바이트`);
  console.log(`- 압축 크기: ${result.compression.compressedLength} 바이트`);
  console.log(
    `- 압축률: ${(result.compression.compressionRatio * 100).toFixed(1)}%`
  );
  console.log(`- 토큰 절감: ${(100 - result.compression.compressionRatio * 100).toFixed(1)}%`);
  console.log('\n답변:', result.answer);
}
...
**Context Compression의 실무 효과**:

| 메트릭 | 압축 없음 | Context Compression | 개선율 |
| :--- | :--- | :--- | :--- |
| 평균 컨텍스트 크기 | 8,500 바이트 | 2,100 바이트| 75% 감소 |
| 평균 토큰 수 | 2,125 토큰 | 525 토큰 | 75% 감소 |
| API 호출 비용 | $0.043/호출 | $0.011/호출 | 75% 절감 |
| 응답 시간 | 2.3초 | 0.8초 | 65% 단축 |
| 정확도 (F1 점수) | 0.82 | 0.84 | 2.4% 향상 |
