---
title: "컨텍스트 스케일링 3-레이어 아키텍처 — 프롬프트 캐싱 · Context Pruning · JIT RAG로 '지능 = 토큰' 선형 트레이드오프 끊기"
category: context-engineering
date: "2026-04-16"
tags: [context-engineering, tokenomics, rag, prompt-caching, context-pruning, jit-retrieval, scaling, architecture, direction-setting]
confidence: 3
connections:
...
status: complete
description: "AI 하네스에 장기/중기 기억(MD 저널·스펙·솔루션)이 누적되면 '컨텍스트 양 = 지능 = 토큰/비용/레이턴시'라는 선형 트레이드오프에 갇히는 듯 보인다. 이는 아키텍처 선택의 문제이지 불가피한 숙명이 아니다. 정적 캐시(Prompt Caching) · Pruning/Compression · JIT Retrieval 3-레이어로 끊는 방법과 ai-study/moneyflow/tarosaju/aidy에의 실전 적용 경로를 제시한다."
type: entry
quiz:
  - question: "'지능 ≈ 컨텍스트 양 ≈ 토큰 비용' 이라는 선형 트레이드오프를 끊는 3-레이어 아키텍처의 본질적 전환은?"
    choices:
      - "더 큰 컨텍스트 윈도우(1M)를 쓰면 된다"
      - "컨텍스트를 '변하지 않음 / 요약 가능 / 휴면 상태'로 분리해서 각각 캐시 · 압축 · 쿼리-시점 동적 주입(JIT) 으로 다르게 다룬다. 한 덩어리로 취급하면 선형 비용을 피할 수 없지만, 분리하면 *필요할 때만* 지불하는 구조가 된다"
      - "모든 문서를 영어로 번역해서 토큰을 줄인다"
      - "Haiku로 모델을 다운그레이드한다"
    answer: 1
    explanation: "문제는 '텍스트가 많다'가 아니라 '매 호출마다 전체를 쌩으로 밀어넣는다' 이다. 세 가지 레이어로 분리하면: (1) 정적 블록은 첫 호출에서만 비용을 지불하고 이후 90% 할인(캐시), (2) 유동 블록은 압축해서 시그널만 유지, (3) 휴면 블록은 평소엔 전혀 로드 안 하고 쿼리가 오면 벡터 DB에서 관련 1~2개만 pull. 결과: 지식량이 N배 늘어도 실제 토큰은 log N 수준으로 증가."
  - question: "'4 조건' (카탈로그 applied-log 기준) 중 '롤백 비용이 낮음' 조건이 JIT Retrieval 도입에 주는 시사점은?"
    choices:
      - "벡터 DB는 쉬우니 무조건 도입"
      - "JIT Retrieval은 아키텍처 변경(외부 임베딩 + 검색 인프라)이므로 '한 줄 env var' 수준의 롤백 비용이 아니며, 따라서 즉시 적용보다는 단계적 POC를 통해 가치를 먼저 실측하고 점진 도입해야 한다"
      - "RAG는 위험하니 쓰면 안 된다"
      - "프롬프트 캐싱과 동일한 난이도"
    answer: 1
    explanation: "프롬프트 캐싱은 cache_control 추가 한 줄 + 롤백도 한 줄. 4 조건 통과 즉시 적용 가능. JIT Retrieval은 외부 서비스(Chroma/Pinecone) + 임베딩 파이프라인 + 쿼리 라우팅 코드 + 동기화 전략이 필요. 4 조건의 '롤백 비용이 낮음' 기준에서 벗어나므로 '실측 기반 POC → 단일 프로덕트 → 이식' 순서를 밟아야 한다. 섀도우 배포(기존 전체 로드와 JIT 주입을 병행, 출력 차이 측정) 후 전환 권장."
  - question: "어떤 지식을 어느 레이어에 둘지 판단하는 가장 실용적인 질문은?"
    choices:
      - "파일 크기가 얼마인가?"
      - "얼마나 오래된 문서인가?"
      - "'이 정보는 (a) 매 호출에 쓰이는가, (b) 자주 참조되지만 매번은 아닌가, (c) 특정 트리거에서만 필요한가' — a는 캐시, b는 요약본 프롬프트 + 원문 링크 또는 인메모리 슬라이딩, c는 벡터 DB JIT 로 분리"
      - "마크다운인가 JSON인가?"
    answer: 2
    explanation: "레이어 분류는 '내용의 성격'이 아니라 '사용 패턴'이 기준이다. CLAUDE.md/SPEC.md는 매 세션 프리픽스 = Layer 1(캐시). 최근 대화 10턴은 (b) = Layer 2(압축/슬라이딩). 과거 솔루션 100개는 '특정 에러 메시지 나왔을 때만 쓸 지식' = Layer 3(JIT). 한 파일이 여러 성격을 섞고 있으면 그 자체가 분리 신호 — 분해해서 각 조각을 적절한 레이어로."
---
...
```mermaid
flowchart TD
    User["유저 프롬프트<br/>(예: 'SPM 순환 참조 에러 났어')"]

    subgraph L1 ["Layer 1 — Prompt Caching (정적)"]
      Cache["CLAUDE.md + SPEC.md + api-contract.md<br/>cache_control: ephemeral (5분 TTL)"]
      CacheHit["캐시 히트: 비용 90% 할인<br/>TTFT 단축"]
    end

    subgraph L2 ["Layer 2 — Pruning / Compression (유동)"]
      Working["현재 세션 대화 이력"]
      Compressor["슬라이딩 윈도우 + 요약기<br/>최근 N턴은 원문 · 그 이전은 압축 요약"]
      Shrunk["시그널만 유지된 맥락"]
    end

    subgraph L3 ["Layer 3 — JIT Retrieval (휴면)"]
      Orchestrator["오케스트레이터<br/>쿼리 분류기"]
      VectorDB["벡터 DB · Chroma/Pinecone<br/>+ 과거 솔루션 · WO · 회고 임베딩"]
      TopK["Top 1~3 엔트리만 pull<br/>(예: SPM 관련 솔루션 2개)"]
      JITInject["현재 프롬프트에 슬쩍 주입<br/>(관련 조각만)"]
    end
...
```mermaid
flowchart LR
    Session["세션 쿼리<br/>('SPM 순환 참조 에러')"]

    subgraph Router ["쿼리 라우터"]
      Classifier["경량 분류기<br/>(Haiku 또는 규칙 기반)"]
      Embedder["임베딩 생성<br/>(Voyage · OpenAI · local)"]
    end

    subgraph Index ["인덱스 저장소"]
      VDB["벡터 DB<br/>Chroma local 또는<br/>Pinecone hosted"]
      Meta["메타데이터<br/>(태그 · 날짜 · 카테고리 · 신뢰도)"]
    end

    subgraph Retrieve ["검색 + 필터"]
      Search["코사인 유사도 Top-K<br/>(K=5~10)"]
      Rerank["Reranker (선택)<br/>→ Top 1~3"]
      Filter["메타 필터<br/>(예: 최근 6개월만)"]
    end

    subgraph Inject ["주입"]
      Shape["선택된 문서 → 요약/발췌"]
      Slot["프롬프트 슬롯에 삽입<br/>(예: '## 관련 과거 사례')"]
    end

    Session --> Classifier
    Classifier -->|검색 필요 플래그| Embedder
    Classifier -->|검색 불필요| SkipIndex["인덱스 미호출<br/>(비용 0)"]
    Embedder --> Search
    Search --> VDB
    VDB --> Rerank
    Meta --> Filter
    ...
    Filter --> Shape
    Shape --> Slot
    Slot --> FinalPrompt["최종 프롬프트<br/>(Layer 1 캐시 + Layer 2 요약 + Layer 3 JIT 조각)"]
```
...
## 6. 통합 의사결정 프레임워크 — '어느 지식을 어느 레이어에?'
