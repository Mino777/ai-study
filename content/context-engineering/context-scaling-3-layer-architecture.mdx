...
status: complete
description: "AI 하네스에 장기/중기 기억(MD 저널·스펙·솔루션·핸드오프)이 누적되면 '컨텍스트 양 = 지능 = 토큰/비용/레이턴시'라는 선형 트레이드오프에 갇히는 듯 보인다. 이는 아키텍처 선택의 문제이지 불가피한 숙명이 아니다. 정적 캐시(Prompt Caching) · Pruning/Compression · JIT Retrieval 3-레이어로 끊는 방법과 '단순 핸드오프 문서는 삭제가 정답' 특별 사례, ai-study/moneyflow/tarosaju/aidy 실전 적용 경로를 제시한다."
type: entry
quiz:
