...
quiz:
  - question: RAG가 필요한 주요 상황은?
    ...
    answer: 1
    explanation: >-
      RAG는 LLM의 학습 데이터에 없는 정보(최신 뉴스, 내부 문서, 실시간 데이터)가 필요한 경우에 사용한다. 일반적인 QA에는
      불필요하지만, 정확성과 신선도가 중요한 경우 매우 효과적이다.
  - question: Chunking 전략 중 의미 단위를 가장 잘 보존하는 방식은?
    ...
    answer: 2
    explanation: >-
      Semantic chunking은 의미 단위를 가장 잘 보존하지만 느리다(각 청크를 임베딩하여 유사도 계산).
      Fixed/Recursive는 빠르지만 의미를 고려하지 않는다. 대부분의 경우 Recursive가 균형잡힌 선택이다.
  - question: RAG의 '손실 중간' (Lost in the Middle) 문제는?
    ...
    answer: 1
    explanation: >-
      LLM은 컨텍스트의 처음과 끝에 더 주의를 기울인다. RAG에서 검색된 문서가 많으면 가장 관련 있는 문서를 맨 앞 또는 맨 뒤에
      배치해야 한다.
applicable_to:
  - any
connections:
