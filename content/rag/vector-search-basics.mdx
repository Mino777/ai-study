...
quiz:
  - question: "RAG의 retrieval 단계에서 코사인 유사도를 주로 사용하는 이유는?"
    ...
    answer: 1
    explanation: "긴 문서와 짧은 문서를 비교할 때 벡터의 크기 차이가 유사도에 영향을 주는 것을 피하기 위해 방향만 비교한다. 코사인 유사도 = (a·b) / (‖a‖‖b‖)는 정규화된 벡터면 내적만으로도 계산 가능하다."
  - question: "k-NN이 아니라 ANN(Approximate Nearest Neighbor)을 사용해야 하는 기본 기준은?"
    choices:
      ...
      - "벡터 차원이 높을 때만"
    answer: 1
    explanation: "100만 문서 × 1536차원이면 한 쿼리에 수십억 곱셈이 필요하다. ANN(HNSW)은 recall 95% 수준에서도 최종 답변 품질에 거의 영향이 없으므로, 속도를 O(log N)으로 줄이는 것이 합리적인 트레이드다."
  - question: "벡터 DB 선택 시 HNSW가 기본 추천되는 이유는?"
    choices:
      ...
      - "메모리 사용량이 가장 적기 때문"
    answer: 1
    explanation: "IVF는 수억 벡터 규모에, LSH는 이론적으로 깔끔하지만 실무 성능이 떨어진다. HNSW는 메모리를 사용하는 대신 균형 잡힌 성능을 제공하므로, 처음 설계할 때 고민 없이 HNSW로 시작하면 된다."
connections:
