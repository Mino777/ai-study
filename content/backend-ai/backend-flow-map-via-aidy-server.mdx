...
status: complete
description: >-
  iOS 개발자가 aidy-server(Spring Boot + Kotlin) 레포로 백엔드를 학습할 때, 코드 디테일이 아닌 '요청 하나의
  여정'을 따라가며 큰 그림을 잡는 학습 경로. 인증/DB/외부 API/에러 처리/운영을 한 묶음으로 이해하기 위한 로드맵과 iOS 대응
  매핑표.
type: entry
quiz:
  - question: 백엔드 큰 그림 학습에서 '요청 하나의 여정'을 따라가는 방식을 가장 먼저 추천하는 이유는?
    choices:
      - 코드 파일 수가 적어서
      - >-
        파일/계층별로 읽으면 각 조각이 왜 필요한지 맥락을 잃기 쉬움. 한 요청(예: POST /api/chat)을 필터 → 컨트롤러 →
        서비스 → DB → 응답까지 관통시키면 모든 조각이 자연스럽게 한 줄기에 꿰인다
      - Spring Boot 공식 문서가 그렇게 추천하기 때문
      - 테스트 작성이 쉬워서
    answer: 1
    explanation: >-
      Controller, Service, Repository, Filter, Exception Handler를 각각 따로 읽으면 '이게
      왜 있어야 하는가'에 대한 감각이 생기지 않는다. 한 요청의 여정은 모든 레이어가 한 번에 호출되는 유일한 관점이라, 큰 그림을 잡는
      가장 빠른 경로다.
  - question: iOS의 'Interceptor/Middleware' 개념과 가장 가까운 Spring Boot 구성 요소는?
    ...
    answer: 1
    explanation: >-
      요청이 Controller에 도달하기 전에 공통 관심사(인증, Request-Id 부여, Rate Limit)를 처리하는 레이어.
      iOS의 Alamofire RequestInterceptor / URLProtocol 스택과 같은 역할. 개별 Controller가
      각자 토큰 검증을 하지 않도록 한다.
  - question: Circuit Breaker가 백엔드에서 '큰 그림' 수준에서 중요한 이유는?
    choices:
      - 코드가 예뻐져서
      - >-
        외부 API(Claude 등)가 일시 장애일 때 무한 재시도가 rate limit 증폭·비용 폭증·사용자 대기를 일으키므로,
        장애를 격리하고 빠르게 503으로 끊어주는 '연쇄 장애 차단기'가 필요하기 때문
      - Spring Boot 필수 기능이라서
      - 테스트 커버리지를 높이기 위해
    answer: 1
    explanation: >-
      단일 장애가 시스템 전체를 주저앉히지 않도록 '의도적 실패 fast-fail'을 만드는 것이 Circuit Breaker.
      aidy-server는 ADR-007에 따라 in-memory CB로 Claude API 의존성을 격리한다. 외부 의존성을 다루는
      모든 백엔드의 공통 주제.
---
...
```mermaid
flowchart TD
    Client["iOS/Android 클라이언트 · POST /api/chat + JWT"] --> F1

    subgraph Filters ["서블릿 필터 체인"]
      F1["RequestIdFilter · 모든 로그에 추적 ID 부여"] --> F2["JwtAuthenticationFilter · 토큰 검증 → 누가 호출했는지 식별"]
    end

    F2 --> I1["RateLimitInterceptor · 분당 호출 제한 · InMemoryRateLimiter"]
    I1 --> C["ChatController · HTTP → Kotlin 함수"]
    C --> S["ChatService · 비즈니스 로직"]

    S --> AI["AiService · Claude API 호출"]
    AI --> CB{"AiCircuitBreaker · CLOSED/OPEN/HALF_OPEN"}
    CB -->|CLOSED| Anthropic[Anthropic Claude API]
    CB -->|OPEN| Fail["CircuitOpenException · → 503 AI_UNAVAILABLE"]

    S --> M["MemoryService · 대화에서 메모리 자동 추출"]
    S --> R["Repository · Spring Data JPA"]
    R --> DB["(PostgreSQL · ChatMessage · Memory)"]

    Anthropic --> S
    DB --> S
    S --> C
    C --> EH{에러 발생?}
    EH -->|예외| GEH["GlobalExceptionHandler · ErrorCode 표준화"]
...
## s27 업데이트 — v2.3~v2.6 Server 구현 (2026-04-19)

s27 autoceo 스프린트에서 4개 피처의 서버 API가 추가되었다. Spring Boot + Kotlin 패턴의 반복 적용.

### 추가된 피처

| 버전 | 피처 | 핵심 Server 패턴 |
|------|------|-----------------|
| v2.3 | Anniversary Reminders API | Controller → Service → Repository 3층 + Flyway 마이그레이션 |
| v2.4 | Notification Preferences API | CRUD 엔드포인트 + 유저별 설정 저장 |
| v2.5 | Relationship Nudges API | AI 분석 결과 → 푸시 알림 트리거 로직 |
| v2.6 | Gift Suggestions API | Claude API 호출 → 추천 결과 캐싱 + 반환 |

### 학습 포인트

- **"서버 먼저" 원칙**: 4개 피처 모두 서버 API가 먼저 완성 → Gate-1 통과 → iOS/Android에 dispatch. api-contract.md가 있으니 서버가 먼저 구현해도 클라이언트가 즉시 연동 가능
- **에지 케이스 테스트 보강 (R6)**: v2.4/v2.5 에지 케이스를 별도 라운드에서 추가
- **최종 테스트**: 796 tests 전체 PASS (3-client 중 가장 많은 테스트 수)
