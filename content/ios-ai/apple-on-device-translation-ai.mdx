...
generated_by: gemini
quiz:
  - question: "TranslationSession을 프로그래밍 방식으로 사용하려면 최소 어느 OS 버전이 필요한가?"
    choices:
      - "iOS 16"
      - "iOS 17.4"
      - "iOS 18"
      - "iOS 26"
    answer: 2
    explanation: "시스템 팝오버 translationPresentation은 iOS 17.4+, 코드로 결과를 직접 받는 TranslationSession + .translationTask는 iOS 18+부터 제공된다."
  - question: "'브라켓(Bracket)' 패턴의 핵심 아이디어는?"
    choices:
      - "모든 언어별로 LLM 로직을 따로 구현한다"
      - "핵심 로직은 영어로만 작성하고 입출력 양 끝단에 번역 계층을 둔다"
      - "번역을 서버에서 처리해 디바이스 부하를 줄인다"
      - "번역 결과를 캐시해 재사용한다"
    answer: 1
    explanation: "Translate → Reason(English) → Translate 구조로, 단일 언어 AI 로직을 번역 계층으로 감싸 다국어로 즉시 확장한다. 전 과정이 온디바이스라 비용·개인정보 문제가 없다."
  - question: "Translation Framework 도입 시 반드시 고려해야 할 현실적 제약은?"
    choices:
      - "유료 API 키 발급이 필요하다"
      - "시뮬레이터에서 동작하지 않으며 최초 번역 시 언어팩 다운로드가 필요하다"
      - "영어 단일 언어만 지원한다"
      - "네트워크가 항상 연결되어 있어야 한다"
    answer: 1
    explanation: "Translation Framework는 시뮬레이터 미지원이라 실기기 테스트가 필수이고, 특정 언어 쌍 최초 번역 시 언어팩 다운로드 대기(availability 체크 + prepare)를 UX로 처리해야 한다."
---

> 이 엔트리는 [Blake Crosley](https://blakecrosley.com/) 블로그의 Apple Translation Framework 분석 글을 정독하고 핵심을 추출한 것이다. Crosley는 Apple의 WWDC 발표와 공식 API 문서를 기반으로 실제 구현 시 마주할 수 있는 함정과 활용 패턴을 다룬다.

> 본 엔트리의 API 시그니처는 Apple 공식 문서로 교차 검증했다 ([TranslationSession](https://developer.apple.com/documentation/translation/translationsession), [translate(batch:)](https://developer.apple.com/documentation/translation/translationsession/translate(batch:)), [Foundation Models](https://developer.apple.com/documentation/FoundationModels)).
