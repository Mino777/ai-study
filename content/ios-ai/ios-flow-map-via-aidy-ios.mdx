...
status: complete
description: >-
  RIBs/ReactorKit 기반 레거시 프로덕션 iOS 앱에 익숙한 개발자가 aidy-ios(TCA + SwiftUI + Tuist)를
  통해 같은 플랫폼을 '다른 아키텍처'로 다시 보며 흐름을 정리한다. Flow Map 시리즈의 역방향 학습 편: 메시지 한 번의 여정을
  View → Action → Reducer → Effect → State → 재렌더로 관통하고, RIBs/ReactorKit ↔ TCA와
  Compose ↔ TCA를 양방향 매핑한다.
type: entry
quiz:
...
Flow Map 시리즈의 **역방향 학습 편**. iOS 프로덕션 앱(RIBs + ReactorKit 기반 레거시 프로덕션 앱)에 익숙한 개발자가 `aidy-ios`(TCA + SwiftUI + Tuist)로 **같은 플랫폼을 다른 아키텍처로 다시** 관통한다. "메시지 한 번의 여정"을 View → Action → Reducer → Effect → State → 재렌더로 따라가면, TCA가 왜 Redux/Elm 계보인지와 Android Compose/Spring Boot 계층이 결국 같은 **단방향 상태 머신** 패턴의 변형임이 선명해진다.
...
`~/Develop/aidy-ios` — Tuist + SPM + SwiftUI + TCA + URLSession + Keychain. 레거시 프로덕션에서 쓰는 RIBs/ReactorKit 조합과 **다른 아키텍처지만 플랫폼은 같다**.
...
flowchart TD
    ...
    subgraph ViewLayer ["View Layer - SwiftUI"]
      CV["ChatView · @Bindable store"]
      CV -->|Button action| DispatchAction["store.send(.sendTapped)"]
    end

    User --> CV

    subgraph ReducerLayer ["Reducer Layer - TCA"]
      DispatchAction --> R[ChatFeature.body]
      R --> Pure["순수 상태 변경 · messages.append / inputText = empty / isLoading = true"]
      Pure --> EffectNode[".run 반환 · 비동기 Effect 실행"]
    end

    subgraph DILayer ["Dependency Injection"]
      EffectNode --> DepRes["@Dependency(apiClient)"]
      DepRes -->|live or test| Live[APIClient.liveValue]
    end

    subgraph NetworkLayer ["Network Layer"]
      Live --> Req["authorizedRequest · Keychain에서 JWT 로드"]
      Req --> HeaderNode["Authorization Bearer token"]
      HeaderNode --> URLS[URLSession.data]
    end

    URLS --> Server["aidy-server · POST /api/chat"]
    Server -->|응답| URLS2[URLSession 응답]
    URLS2 --> CheckResp{"checkResponse · status code"}
    CheckResp -->|401| Notif["NotificationCenter · unauthorizedDetected post"]
    CheckResp -->|4xx/5xx| APIErr["APIError.server · message, code"]
    CheckResp -->|200-299| DecodeNode["JSONDecoder · ChatResponse"]

    DecodeNode --> ResultNode[Effect 완료]
    APIErr --> ResultNode
    Notif --> ResultNode

    ResultNode --> Back["send(.chatResponse(Result))"]
    Back --> R2[Reducer 재실행]
    R2 --> StateUpdate["State 업데이트 · isLoading = false / messages 추가"]
    StateUpdate --> Obs["@ObservableState · 변경 감지"]
    Obs --> SwiftUI["SwiftUI 재렌더 · diff-based update"]
...
- **`authorizedRequest`**: Keychain에서 토큰 읽어 Bearer 헤더 자동 첨부. 프로덕션 프로젝트의 Alamofire Interceptor 대응.
...
### Week 5 — 역방향 비교 학습
- 레거시 프로덕션 프로젝트에서 유사 기능 하나 고르기 (예: 로그인)
...
- [iOS Harness Journal 000 — 베이스라인](/wiki/ios-ai/ios-ai-journal-000-baseline) — 레거시 프로덕션 iOS 앱에 AI 하네스 설치
