...
applicable_to: ["any"]
connections: [ios-ai/apple-intelligence-api, ios-ai/coreml-on-device-inference-patterns]
description: "iOS 26의 파운데이션 모델에서 커스텀 어댑터를 훈련해야 하는 시기와 그 방법을 설명합니다."
generated_by: gemini
quiz:
  - question: "Apple이 커스텀 어댑터를 도입하기 전에 먼저 시도하라고 권고하는 것은?"
    choices:
      - "전체 모델 재학습"
      - "프롬프트 엔지니어링과 Tool Calling"
      - "더 큰 클라우드 모델로 교체"
      - "어댑터 파일 압축"
    answer: 1
    explanation: "Apple은 어댑터를 마지막 수단으로 규정하고, 프롬프트 엔지니어링과 Tool Calling이 실패했을 때만 고려하라고 권고한다."
  - question: "Apple의 커스텀 어댑터가 사용하는 미세조정 기술은?"
    choices:
      - "전체 파라미터 미세조정(Full Fine-Tuning)"
      - "RLHF"
      - "LoRA(Low-Rank Adaptation) 기반 PEFT"
      - "Knowledge Distillation"
    answer: 2
    explanation: "어댑터는 시스템 모델 가중치를 동결하고 작은 LoRA 행렬만 학습하는 PEFT 방식을 사용한다."
  - question: "어댑터(.fmadapter)의 가장 큰 운영상 제약은?"
    choices:
      - "Wi-Fi에서만 동작"
      - "특정 시스템 모델 버전과 1:1로 종속되어 OS 업데이트 시 재학습 필요"
      - "앱당 1개만 허용"
      - "영어만 지원"
    answer: 1
    explanation: "어댑터는 특정 시스템 모델 버전과만 호환되며, Apple이 새 모델을 배포하면 재학습·재배포가 필수다."
---
...
- **지속적인 유지보수**: 이는 어댑터를 위한 지속적인 데이터셋 관리, 학습, 평가, 배포 파이프라인 구축이 필수적임을 의미한다.
...
graph TD
    ...
    subgraph AdapterLifecycleWhen["Adapter Lifecycle When 'No'"]
