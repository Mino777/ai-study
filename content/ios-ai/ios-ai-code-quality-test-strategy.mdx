...
status: complete
description: >-
  iOS 앱에서 AI가 생성하거나 수정한 코드의 신뢰성을 확보하기 위한 유닛, 통합, UI, E2E 테스트 환경 구성 및 실전
  Swift/SwiftUI 패턴.
type: entry
quiz:
  - question: AI 생성 코드에 특별한 테스트 전략이 필요한 주요 이유가 아닌 것은 무엇인가요?
    ...
    answer: 3
    explanation: >-
      본문에서는 AI 생성 코드의 '예측 불가능성과 잠재적 버그'를 주요 도전 과제로 언급하며, AI 모델의 환각, 비결정론적 특성,
      사람이 놓치기 쉬운 미묘한 비일관성 등이 특별한 테스트 전략이 필요한 이유라고 명시하고 있습니다. AI 생성 코드가 문법 오류가 적을
      수 있다는 점은 생산성 향상에 기여할 수 있으나, 특별한 테스트 전략의 필요성을 제기하는 핵심적인 이유와는 관련이 없습니다.
  - question: >-
      본문에서 재정의된 iOS AI 코드 테스트 피라미드에서, AI의 고유한 특성을 고려하여 새로 추가되거나 강화된 계층과 그 역할에 대한
      설명으로 올바르지 않은 것은?
    choices:
      - >-
        AI Agent Input Validation (최하단): AI 에이전트에 전달되는 프롬프트나 인자가 유효한지 사전 검증하여 AI
        시스템의 견고성을 높인다.
      - >-
        Unit & AI Output Schema Validation: AI가 생성한 코드 단위의 기능 검증과 더불어, AI의 중간
        출력이 유효한 스키마를 따르는지 검증하여 AI의 환각을 초기에 방지한다.
      - >-
        Integration & Contract Testing: AI 생성 모듈과 기존 시스템 간의 연동을 확인하고, 비동기 로직 및
        데이터 흐름을 검증한다.
      - >-
        E2E & Human-in-the-Loop: 주로 자동화된 테스트를 통해 AI 생성 코드의 최종적인 사용자 경험을 검증하며,
        복잡한 워크플로우에서는 사람의 개입을 최소화한다.
    answer: 3
    explanation: >-
      본문에서 E2E & Human-in-the-Loop 계층에 대해 '복잡한 AI 에이전트 워크플로우의 경우 사람의 개입을 통한 최종
      검토가 필수적입니다'라고 명시되어 있습니다. 따라서, 사람의 개입을 최소화한다는 설명은 본문의 내용과 다릅니다. 다른 선택지들은
      본문의 내용과 일치합니다.
  - question: >-
      AI 생성 Swift 코드의 유닛 테스트 시 '결정론적 환경' 구축과 'AI 출력 스키마 유효성' 검증을 위한 전략으로 가장 적절한
      설명을 모두 고른 것은?
    choices:
      - 비결정론적 요소를 가진 AI 생성 함수는 테스트 대상에서 제외한다.
      - >-
        Mocking/Stubbing을 통해 Date나 UUID 같은 요소를 고정된 값으로 주입하고, Codable과 커스텀 로직으로
        AI 중간 출력 스키마를 검증한다.
      - XCTestExpectation을 활용하여 비결정론적 요소의 모든 가능한 출력을 검증한다.
      - 'AI 출력 스키마 검증은 통합 테스트 단계에서만 수행하며, 유닛 테스트에서는 오직 기능 검증에 집중한다.'
    answer: 1
    explanation: >-
      본문은 유닛 테스트에서 '결정론적 환경' 구축을 위해 Date나 UUID 같은 비결정론적 요소를 Mocking/Stubbing을 통해
      고정된 값으로 주입해야 한다고 설명합니다. 또한 'AI 출력 스키마 유효성 검증'을 위해 Codable 프로토콜과 커스텀 유효성 검증
      로직을 활용하여 JSON 등의 중간 출력을 검증한다고 명시하고 있습니다. 다른 선택지들은 본문의 설명과 일치하지 않거나 핵심 전략이
      아닙니다.
generated_by: gemini
---
