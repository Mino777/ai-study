...
quiz:
  - question: 본문에 설명된 '마스터 컨트롤 프로그램(MCP)' 아키텍처에서 LLM(Claude)의 핵심적인 역할은 무엇인가요?
    ...
    answer: 1
    explanation: >-
      본문에서 MCP는 LLM을 '번역가'이자 '실행 계획 수립자'로 사용한다고 명시합니다. 즉, 개발자의 자연어 요청(예: "유닛 테스트
      돌려줘")을 받아, 미리 정의된 `xcodebuild` 도구 사용법에 맞춰 정확한 파라미터를 포함한 JSON 형식의 커맨드로 변환하는
      것이 LLM의 핵심 역할입니다. 1번은 본문에서 언급되지 않은 기능입니다. 3번과 4번은 실제 커맨드 실행 및 환경과 관련된
      부분으로, 이는 MCP의 Swift 스크립트와 `Process` API가 담당하는 역할이지 LLM의 역할이 아닙니다.
  - question: MCP가 Claude를 이용해 정확한 `xcodebuild` 커맨드를 생성하도록 유도하기 위해 사용된 가장 중요한 방법은 무엇인가요?
    choices:
      ...
      - >-
        `xcodebuild`의 파라미터, 설명, 예시 등을 상세히 기술한 프롬프트를 제공하여 원하는 JSON 형식을 명확히 요구하는
        방식
    answer: 3
    explanation: >-
      본문의 'LLM에 `xcodebuild` 도구 정의 알려주기' 섹션에서 자세히 설명하듯이, MCP는 시스템 프롬프트를 통해
      `xcodebuild`라는 도구의 정의, 각 파라미터의 종류와 설명, 그리고 최종적으로 반환해야 할 JSON의 구조까지 명확하게
      제시합니다. 이를 통해 LLM이 사용자의 의도를 정확한 커맨드로 변환하도록 유도합니다. 본문은 프롬프트 엔지니어링 기법을 설명하며,
      1, 2, 3번과 같은 방식은 언급하지 않았습니다. 특히 팁 부분에서 `xcodebuild -list` 결과를 프롬프트에 동적으로
      포함하는 것을 추천하는데, 이는 4번 방식의 연장선상에 있습니다.
  - question: 제시된 Swift 코드 예시에서 `Process` API의 주된 용도는 무엇인가요?
    ...
    answer: 1
    explanation: >-
      본문의 `XcodeBuildRunner.swift` 코드에서 `Process`는 `/usr/bin/xcodebuild`를 실행
      URL로 설정하고, LLM이 생성한 파라미터(`arguments`)를 받아 외부 프로세스로 실행하는 데 사용됩니다. 또한 실행 후
      표준 출력(stdout), 표준 에러(stderr), 종료 코드(terminationStatus)를 수집하여 반환하는 역할을 합니다.
      1번은 `JSONDecoder`의 역할이고, 3번은 `URLSession`과 같은 네트워킹 라이브러리의 역할입니다. 4번도
      `Process`를 통해 `xcodebuild -list`를 실행하여 구현할 수 있지만, MCP의 메인 워크플로우에서
      `Process`의 핵심 역할은 LLM이 계획한 명령을 '실행'하는 것입니다.
generated_by: gemini
---
